    ...
    <div style={{ color: 'var(--text0)', paddingBottom: 24 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20, flexWrap: 'wrap', gap: 10 }}>
        ...
        <button onClick={() => setShowAddForm(v => !v)} style={{
          padding: '8px 16px', borderRadius: 16, cursor: 'pointer', fontSize: 12,
      ...
      {showAddForm && (
        <div style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 10, marginBottom: 10 }}>
            <div>
              <label style={{ display: 'block', fontSize: 9, fontFamily: "'JetBrains Mono',monospace", letterSpacing: 1, color: 'rgba(200,164,90,0.6)', marginBottom: 4 }}>НАЗВАНИЕ *</label>
              <input value={docForm.title} onChange={e => setDocForm(p => ({ ...p, title: e.target.value }))}
                placeholder="Акт сверки с ТОО..."
                style={{ width: '100%', padding: '9px 11px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, color: 'var(--text0)', fontSize: 13, outline: 'none', boxSizing: 'border-box' }}
              />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: 9, fontFamily: "'JetBrains Mono',monospace", letterSpacing: 1, color: 'rgba(200,164,90,0.6)', marginBottom: 4 }}>ТИП</label>
            ...
            <div>
              <label style={{ display: 'block', fontSize: 9, fontFamily: "'JetBrains Mono',monospace", letterSpacing: 1, color: 'rgba(200,164,90,0.6)', marginBottom: 4 }}>ПУТЬ</label>
            ...
            <div>
              <label style={{ display: 'block', fontSize: 9, fontFamily: "'JetBrains Mono',monospace", letterSpacing: 1, color: 'rgba(200,164,90,0.6)', marginBottom: 4 }}>ТЕГИ (через запятую)</label>
          ...
          <div style={{ marginBottom: 12 }}>
            <label style={{ display: 'block', fontSize: 9, fontFamily: "'JetBrains Mono',monospace", letterSpacing: 1, color: 'rgba(200,164,90,0.6)', marginBottom: 4 }}>ЗАМЕТКИ</label>
            <textarea value={docForm.notes} onChange={e => setDocForm(p => ({ ...p, notes: e.target.value }))}
              placeholder="Дополнительные сведения..."
              rows={2}
              style={{ width: '100%', padding: '9px 11px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, color: 'var(--text0)', fontSize: 13, resize: 'vertical', outline: 'none', boxSizing: 'border-box' }}
            />
          </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
        <input
          ...
          style={{
            ...
            borderRadius: 20, color: 'var(--text0)', fontSize: 13, outline: 'none',
          }}
        />
        <button onClick={performSemanticSearch} disabled={loadingSearch} style={{
          ...
          fontSize: 13, opacity: loadingSearch ? 0.7 : 1,
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 14, overflowX: 'auto', paddingBottom: 4 }}>
        <div onClick={() => setFilterType('all')}
          style={{
            flexShrink: 0, padding: '4px 12px', borderRadius: 10, cursor: 'pointer', fontSize: 10,
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{
                        fontSize: 13, color: 'var(--text1)', fontWeight: 500,
                    ...
                    <div style={{ marginTop: 8, display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                      {doc.tags.map(tag => (
                        <span key={tag} style={{
                          ...
                          border: '1px solid rgba(200,164,90,0.2)',
                          fontSize: 9, color: 'rgba(200,164,90,0.7)',
                          fontFamily: "'JetBrains Mono',monospace",
                        }}>#{tag}</span>
                      ))}
                    </div>
        ...
        <div style={{
          ...
          {selectedDoc ? (
            <>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
                ...
                <button onClick={() => deleteDoc(selectedDoc.id)}
                  style={{
                    ...
                    color: 'rgba(239,68,68,0.6)', fontSize: 11, flexShrink: 0,
                  }}
                >🗑</button>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div style={{
                  ...
                  <div style={{ fontSize: 9, fontFamily: "'JetBrains Mono',monospace", letterSpacing: 1, color: 'rgba(255,255,255,0.3)', marginBottom: 4 }}>ПУТЬ</div>
                ...
                <div style={{
                  ...
                  <div style={{ fontSize: 9, fontFamily: "'JetBrains Mono',monospace", letterSpacing: 1, color: 'rgba(255,255,255,0.3)', marginBottom: 4 }}>ДАТА ДОБАВЛЕНИЯ</div>
                  <div style={{ fontSize: 12, color: 'var(--text2)', fontFamily: "'JetBrains Mono',monospace" }}>
                    {selectedDoc.date}
                  </div>
                </div>
                ...
                {selectedDoc.tags.length > 0 && (
                  <div style={{
                    ...
                    <div style={{ fontSize: 9, fontFamily: "'JetBrains Mono',monospace", letterSpacing: 1, color: 'rgba(255,255,255,0.3)', marginBottom: 6 }}>ТЕГИ</div>
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                      {selectedDoc.tags.map(tag => (
                        <span key={tag} style={{
                          ...
                          background: 'rgba(200,164,90,0.08)',
                          border: '1px solid rgba(200,164,90,0.25)',
                          fontSize: 11, color: 'rgba(200,164,90,0.8)',
                        }}>#{tag}</span>
                      ))}
                    </div>
                  </div>
                ...
                {selectedDoc.notes && (
                  <div style={{
                    ...
                    <div style={{ fontSize: 9, fontFamily: "'JetBrains Mono',monospace", letterSpacing: 1, color: 'rgba(255,255,255,0.3)', marginBottom: 4 }}>ЗАМЕТКИ</div>
            ...
            <div style={{
              ...
              fontFamily: "'Cormorant Infant',serif", fontSize: 13, lineHeight: 1.8,
            }}>
