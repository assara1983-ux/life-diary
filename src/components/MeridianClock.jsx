...
export function MeridianClock({ current, profile, onOpen, size = 240 }) {
  ...
  const getSegmentPath = (index, outerR = 82, innerR = 38) => {
    ...
    const x1 = 100 + outerR * Math.cos(startRad), y1 = 100 + outerR * Math.sin(startRad);
    const x2 = 100 + outerR * Math.cos(endRad), y2 = 100 + outerR * Math.sin(endRad);
    const x3 = 100 + innerR * Math.cos(endRad), y3 = 100 + innerR * Math.sin(endRad);
    const x4 = 100 + innerR * Math.cos(startRad), y4 = 100 + innerR * Math.sin(startRad);
  ...
  return (
    <div className="mc-wrap">
      <style>{`
        ...
        .mc-seg.active { fill: rgba(0,112,192,0.22); stroke: var(--blue); stroke-width: 1.5; }
        .mc-label { font-size: 8px; fill: var(--text2); font-family: var(--font-mono); text-anchor: middle; dominant-baseline: middle; pointer-events: none; }
      ...
      <svg viewBox="0 0 200 200" className="mc-svg">
        <circle cx="100" cy="100" r="96" fill="rgba(245,240,225,0.6)" stroke="var(--line)" strokeWidth="1" />
        {Array.from({ length: 12 }).map((_, i) => {
          const rad = ((i * 30 - 90) * Math.PI) / 180;
          return <line key={`line-${i}`} x1={100 + 38 * Math.cos(rad)} y1={100 + 38 * Math.sin(rad)} x2={100 + 96 * Math.cos(rad)} y2={100 + 96 * Math.sin(rad)} stroke="var(--line)" strokeWidth="0.5" opacity="0.5" />;
        })}
        {MERIDIANS.map((m, i) => {
          const isActive = m.id === activeMeridian.id;
          const midAngle = ((i + 0.5) * 30 - 90) * (Math.PI / 180);
          const lx = 100 + 64 * Math.cos(midAngle), ly = 100 + 64 * Math.sin(midAngle);
          const tx = 100 + 52 * Math.cos(midAngle), ty = 100 + 52 * Math.sin(midAngle);
          return (
        ...
        <circle cx="100" cy="100" r="10" className="mc-center" />
        <circle cx="100" cy="100" r="5" className="mc-dot" />
        <text x="100" y="118" textAnchor="middle" fontSize="9" fill="var(--text2)" fontFamily="var(--font-mono)" fontWeight="600">ЦИ</text>
      </svg>
    </div>
  );
}
