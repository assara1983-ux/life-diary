...
/**
 * Стихия по году рождения: 五行 — пять первоэлементов
 */
...
/**
 * Конституция по сезону рождения
 */
...
/**
 * Полный ТКМ-профиль на основе всех ответов пользователя
 */
export function getTCMFullProfile(profile) {
  ...
  const birthOrgan = profile.birthHour
    ? profile.birthHour.split("—")[1]?.trim().replace("Нет", "")
    : null;

  // Орган из преобладающей эмоции
  const emotionOrgan =
    profile.tcmEmotion?.includes("Дерево") ? "Печень" :
  ...
  // Орган из сна
  const sleepOrgan =
    profile.tcmSleep?.includes("1–3") ? "Печень" :
    profile.tcmSleep?.includes("3–5") ? "Лёгкие" :
    profile.tcmSleep?.includes("5–7") ? "Толстый кишечник" : null;

  // Орган из вкуса
  const tasteOrgan =
    profile.tcmTaste?.includes("Кислое") ? "Печень/Желчный пузырь" :
  ...
  // Пищеварение
  const digestionNote =
    profile.tcmDigestion?.includes("Вздутие") ? "Застой Ци в Желудке" :
  ...
  if (syndromes.includes("Избыток Ян / Жар"))
    foodRecs += ". Избегай: острого, жареного, алкоголя. Добавь: огурец, арбуз, листовой салат, зелёный чай";
  if (syndromes.includes("Недостаток Ян / Холод"))
    foodRecs += ". Добавь: имбирь, корицу, тушёные блюда, тёплые супы. Избегай: сырого, холодного";
  if (syndromes.includes("Сырость-Слизь"))
    foodRecs += ". Избегай: молочного, сладкого, жирного. Добавь: редис, пшено, ячмень, имбирь";
  if (syndromes.includes("Недостаток Инь / Сухость"))
    foodRecs += ". Добавь: кунжут, мёд, груша, лилейный луковица, чёрный кунжут";
...
/**
 * Сезонные ТКМ-рекомендации с учётом возраста и стихии
 */
export function getSeasonalTCM(profile) {
  ...
  // Взаимодействие стихии пользователя с сезоном
  let interaction = "";
  if (el) {
    if (el.name === s.element) {
      interaction = "🔥 Твоя стихия совпадает с сезоном — двойная энергия. Будь внимателен к переизбытку.";
    } else if (el.yin) {
      interaction = "⚡ Твоя стихия Инь — в этом сезоне важно поддерживать тепло и восстановление.";
    } else {
      interaction = "⚡ Твоя стихия Ян — в этом сезоне направь энергию в созидание.";
    }
  }

  const ageNote =
    age < 30 ? "В твоём возрасте энергия Ци сильна — используй сезон для активного роста." :
...
/**
 * Активный меридиан по часу суток
 */
...
/**
 * ТКМ-характеристика дня недели (пять стихий)
 */
...
/**
 * Персональные ТКМ-рекомендации на конкретный день
 */
export function getTCMDayRecs(profile, dayInfo) {
  ...
  // Пересечение органов дня и уязвимых органов пользователя
  uniqueOrgans.forEach(organ => {
    const organLower = organ.toLowerCase().split("/")[0].trim();
    if (dayOrgans.includes(organLower)) {
      recs.push({
        type: "warn",
        text: `Сегодня активен орган «${organ}» — твоя зона внимания. Береги себя.`,
      });
    }
  });
  ...
  if (syndromes.includes("Избыток Ян / Жар") && dayInfo.element === "Огонь")
    recs.push({ type: "warn", text: "Огненный день + твой Жар = риск перегрева. Избегай острого и стресса, пей воду." });
  if (syndromes.includes("Недостаток Ян / Холод") && dayInfo.element === "Вода")
    recs.push({ type: "warn", text: "Водный день + твой Холод = вялость. Имбирный чай и тёплая еда." });
  if (syndromes.includes("Сырость-Слизь") && dayInfo.element === "Земля")
    recs.push({ type: "warn", text: "Земной день + Сырость = тяжесть. Пропусти молочное и сладкое." });

  // Усиление при совпадении стихий
  if (el && el.name === dayInfo.element)
    recs.push({ type: "good", text: `Твоя стихия ${el.name} совпадает с днём — ты в потоке! Используй для важного.` });
