...
import { WesternZodiacIcons, EasternZodiacIcons } from "../components/BlueprintAvatars"; // Используем тот же файл для иконок
import { MeridianClock, LifeCycleSpiral } from "../components/BlueprintIcons";
// ─── ВСПОМОГАТЕЛЬНЫЕ КОМПОНЕНТЫ (Аккордеон и Карточка) ───
...
const { profile } = useApp();const [activeTab, setActiveTab] = useState("about");
...
<div style={{ fontFamily: "var(--font-head)", fontSize: 32, color: "var(--gold)", fontWeight: 600 }}>{insights.destiny?.degree || 241}°</div>          <div style={{ fontFamily: "var(--font-italic)", fontSize: 12, color: "var(--text3)", marginTop: 8 }}>{insights.destiny?.interpretation}</div>
</div>
</div>
{/* ТАБЫ */}
...
{activeTab === "about" && (
<div>
<InsightCardtitle="Градус Судьбы"
icon="🧭"
...
<div style={{ fontSize: 18, fontFamily: "var(--font-head)", color: "var(--gold-dark)", marginBottom: 16 }}>🌙 ЛУННЫЙ ЦИКЛ</div>            <div style={{ fontSize: 14, lineHeight: 1.6, color: "var(--text2)" }}>
...
<div><strong>Рекомендация:</strong> {moonDay > 25 ? "Завершай начатое." : moonDay > 15 ? "Время очищения." : "Время роста и новых начинаний."}</div>
</div>
</div>
