...
// ─── ТОЧНЫЕ ПУТИ К РИСУНКАМ (соответствуют вашим файлам на GitHub) ───
...
// ─── ВНУТРЕННИЙ АККОРДЕОН (для контента внутри перевёрнутых карточек) ───
...
// ─── ПЕРЕВОРАЧИВАЮЩАЯСЯ КАРТОЧКА (Blueprint Style) ───function FlipCardBlock({ title, frontImage, accentColor = "var(--blue)", children, minHeight = 340 }) {
    ...
    <div className="page" style={{ paddingBottom: 100 }}>
      ...
      <FlipCardBlock title="Профиль" frontImage={null} accentColor="var(--blue)" minHeight={360}>
        <div style={{ textAlign: "center", marginTop: 10, marginBottom: 16 }}>
          <div style={{ width: 100, height: 100, margin: "0 auto 12px", borderRadius: "50%", overflow: "hidden", border: "2px solid var(--bg)", boxShadow: "0 4px 12px rgba(0,112,192,0.15)", background: "#fff", display: "flex", alignItems: "center", justifyContent: "center" }}>
            {isMale ? <MaleAvatar size={100} /> : <FemaleAvatar size={100} />}          </div>
      ...
      <FlipCardBlock title="Восточный Знак" frontImage={getFrontImage("eastern", insights.eastern)} accentColor="var(--gold)">
        <div style={{ fontSize: 14, lineHeight: 1.75, color: "var(--text2)" }}>
          ...
          <InnerAccordion title="Энергетический портрет" defaultOpen={true}>            {insights.easternTraits || "Честность и терпимость"}. Твоя стихия наделяет тебя глубокой интуицией и способностью видеть скрытые мотивы.
          </InnerAccordion>
        ...
        <div style={{ fontSize: 14, lineHeight: 1.75, color: "var(--text2)" }}>
          ...
          <div style={{ display: "grid", gap: 10, marginBottom: 14 }}>
            ...
            <div style={{ padding: 12, background: "rgba(139,32,32,0.06)", borderRadius: 8, borderLeft: "3px solid var(--error)" }}>
              ...
              <p style={{ margin: 0, fontSize: 13 }}>{chronoPeaks.rest?.tip || "Идеально для рутины и делегирования."}</p>
            </div>
          </div>
          <InnerAccordion title="Как использовать" defaultOpen={true}>            <ul style={{ margin: "0 0 0 18px", lineHeight: 1.7 }}>
