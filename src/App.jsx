...
function getProfDeadlines(profile) {
  ...
  //
  //  ТОЧНЫЕ СРОКИ ПО НК РК:
  //  ФНО 200.00 — СДАЧА: до 15-го числа 2-го месяца после квартала
  //    1 кв.(янв-март) → 15 мая
  //    2 кв.(апр-июн)  → 15 августа
  //    3 кв.(июл-сен)  → 15 ноября
  //    4 кв.(окт-дек)  → 15 февраля следующего года
  //  ФНО 200.00 — УПЛАТА (ИПН+ОПВ+ОСМС+СО): до 25-го того же месяца
  //    1 кв. → 25 мая | 2 кв. → 25 августа | 3 кв. → 25 ноября | 4 кв. → 25 февраля
  //  ФНО 300.00 (НДС) — СДАЧА и УПЛАТА: до 25-го 2-го месяца после квартала
  //    1 кв. → 25 мая | 2 кв. → 25 августа | 3 кв. → 25 ноября | 4 кв. → 25 февраля
  //  ФНО 910.00 — СДАЧА: до 15-го, УПЛАТА: до 25-го (август / февраль)
  //  ФНО 100/220/240 — СДАЧА: 31 марта, УПЛАТА: 10 апреля
  //  ФНО 700/870 — СДАЧА: 31 марта, УПЛАТА: 1 апреля
  //  Авансы имущество/земля: 25 февр., 25 мая, 25 авг., 25 нояб.
  //  Авансы КПН: 25-го каждого месяца
  // ══════════════════════════════════════════════════════
  if(prof.includes("Бухгалтер") || prof.includes("ИП")) {

    // ── МАЙ 2026 ─────────────────────────────────────
    tasks.push(mk("🏛 КГД: ФНО 200.00 — СДАЧА (1 кв. 2026)",          5, 15,
      "Срок сдачи декларации по ИПН, ОПВ, ВОСМС, ООСМС, СО за янв-март. Ст.209 НК РК","КГД"));
    tasks.push(mk("🏛 КГД: ФНО 300.00 — НДС, СДАЧА+УПЛАТА (1 кв. 2026)", 5, 25,
      "Декларация по НДС за янв-март + уплата НДС одновременно. Ст.424 НК РК","КГД"));
    tasks.push(mk("💰 КГД: ФНО 200.00 — УПЛАТА (1 кв. 2026)",         5, 25,
      "Уплата ИПН + ОПВ + ВОСМС + ООСМС + СО за янв-март 2026. Ст.209 НК РК","КГД"));
    tasks.push(mk("💰 КГД: Аванс налог на имущество/землю (2-й срок)", 5, 25,
      "2-й из 4 авансовых платежей — 1/4 от годовой суммы. Ст.512, 523 НК РК","КГД"));
    tasks.push(mk("💰 КГД: Аванс по КПН — май 2026",                   5, 25,
      "Ежемесячный аванс по корпоративному подоходному налогу. Ст.305 НК РК","КГД"));
    ...
    tasks.push(mk("💰 КГД: Аванс по КПН — июнь 2026",                  6, 25,
      "Ежемесячный авансовый платёж по КПН","КГД"));
    tasks.push(mk("📋 Годовое собрание ТОО / акционеров АО",           6, 30,
      "Обязательно — не позднее 6 месяцев после конца финансового года. ЗРК «О ТОО» ст.45","МЮ"));

    // ── ИЮЛЬ 2026 ────────────────────────────────────
    tasks.push(mk("📊 БНС: Форма 1-Т (2 кв. 2026) — Труд и зарплата", 7, 10,
      "Численность и з/п за 2 квартал (апрель-июнь) 2026","БНС"));
    tasks.push(mk("📊 БНС: Форма 1-П — Промышленность (июль 2026)",    7, 10,
      "Ежемесячный отчёт за июль 2026 (для пром. предприятий)","БНС"));
    tasks.push(mk("💰 КГД: Аванс по КПН — июль 2026",                  7, 25,
      "Ежемесячный авансовый платёж по КПН","КГД"));

    // ── АВГУСТ 2026 ───────────────────────────────────
    tasks.push(mk("📊 БНС: Форма 1-П — Промышленность (август 2026)",  8, 10,
      "Ежемесячный отчёт за август 2026","БНС"));
    tasks.push(mk("🏛 КГД: ФНО 200.00 — СДАЧА (2 кв. 2026)",          8, 15,
      "Срок сдачи декларации по ИПН, ОПВ, ВОСМС, ООСМС, СО за апр-июнь. Ст.209 НК РК","КГД"));
    tasks.push(mk("🏛 КГД: ФНО 910.00 — СДАЧА (1 п/г 2026)",          8, 15,
      "Упрощённая декларация за январь-июнь 2026. Ст.688 НК РК","КГД"));
    tasks.push(mk("🏛 КГД: ФНО 300.00 — НДС, СДАЧА+УПЛАТА (2 кв. 2026)", 8, 25,
      "Декларация по НДС за апр-июнь + уплата НДС. Ст.424 НК РК","КГД"));
    tasks.push(mk("💰 КГД: ФНО 200.00 — УПЛАТА (2 кв. 2026)",         8, 25,
      "Уплата ИПН + ОПВ + ВОСМС + ООСМС + СО за апр-июнь 2026","КГД"));
    tasks.push(mk("💰 КГД: ФНО 910.00 — УПЛАТА (1 п/г 2026)",         8, 25,
      "Уплата ИПН/КПН + соцналог по упрощённой декларации за 1-е полугодие","КГД"));
    tasks.push(mk("💰 КГД: Аванс налог на имущество/землю (3-й срок)", 8, 25,
      "3-й из 4 авансовых платежей — 1/4 от годовой суммы. Ст.512, 523 НК РК","КГД"));
    tasks.push(mk("💰 КГД: Аванс по КПН — август 2026",                8, 25,
      "Ежемесячный авансовый платёж по КПН","КГД"));

    // ── СЕНТЯБРЬ 2026 ─────────────────────────────────
    tasks.push(mk("📋 Воинский учёт — годовая сверка со списками ВЦУ",  9,  1,
      "Ежегодная сверка списков военнообязанных с военкоматом","HR"));
    ...
    tasks.push(mk("📊 БНС: Форма 1-Т (3 кв. 2026) — Труд и зарплата", 10, 10,
      "Численность и з/п за 3 квартал (июль-сентябрь) 2026","БНС"));
    tasks.push(mk("💰 КГД: Аванс по КПН — октябрь 2026",               10, 25,
      "Ежемесячный авансовый платёж по КПН","КГД"));

    // ── НОЯБРЬ 2026 ───────────────────────────────────
    tasks.push(mk("📊 БНС: Форма 1-П — Промышленность (ноябрь 2026)",  11, 10,
      "Ежемесячный отчёт за ноябрь 2026","БНС"));
    tasks.push(mk("🏛 КГД: ФНО 200.00 — СДАЧА (3 кв. 2026)",          11, 15,
      "Срок сдачи декларации по ИПН, ОПВ, ВОСМС, ООСМС, СО за июль-сен. Ст.209 НК РК","КГД"));
    tasks.push(mk("🏛 КГД: ФНО 300.00 — НДС, СДАЧА+УПЛАТА (3 кв. 2026)", 11, 25,
      "Декларация по НДС за июль-сентябрь + уплата НДС. Ст.424 НК РК","КГД"));
    tasks.push(mk("💰 КГД: ФНО 200.00 — УПЛАТА (3 кв. 2026)",         11, 25,
      "Уплата ИПН + ОПВ + ВОСМС + ООСМС + СО за июль-сен 2026","КГД"));
    tasks.push(mk("💰 КГД: Аванс налог на имущество/землю (4-й срок)", 11, 25,
      "Последний (4-й) авансовый платёж — 1/4 от годовой суммы. Ст.512, 523 НК РК","КГД"));
    tasks.push(mk("💰 КГД: Аванс по КПН — ноябрь 2026",               11, 25,
      "Ежемесячный авансовый платёж по КПН","КГД"));

    // ── ДЕКАБРЬ 2026 ──────────────────────────────────
    tasks.push(mk("🏛 КГД: Проверка и продление лицензий",             12,  1,
      "Проверить сроки действия всех лицензий — заблаговременно до истечения","КГД"));
    tasks.push(mk("📋 График отпусков на 2027 год",                    12, 15,
      "Утвердить до 15 декабря включительно. ТК РК ст.93","HR"));
