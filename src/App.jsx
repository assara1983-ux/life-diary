...
function isDue(task, today) {
  ...
  if (task.doneDate===today) return false; // уже выполнено сегодня

  // Beauty-задача с конкретной датой (freq every:14+, deadline задан)
  // Показываем ТОЛЬКО в день deadline
  if (task.section==="beauty" && task.deadline && task.freq && task.freq.startsWith("every:")) {
    const n = parseInt(task.freq.split(":")[1]);
    if (n >= 14) {
      // Нечастые — только в день deadline
      return task.deadline === today;
    }
    // Частые every:2, every:3 — считаем от beautyStartDate если есть
...
function AiBox({ kb, prompt, label="ИИ-СОВЕТНИК", btnText="Получить совет", placeholder="Нажми — получи персональный совет...", actionType=null, onShopAdd=null, onTaskAdd=null, noActions=false, maxTokens=1200, onCreateTool=null }) {
  ...
  // Умное извлечение заголовка — ищем название метода/техники/инструмента
  const getItemTitle = (item) => {
    ...
    // 4. «метод X», «техника X», «правило X» и т.д.
    ...
    // 5. Первая фраза до точки/двоеточия/тире
              ...
              return(
                <div key={ev.id} style={{display:"flex",alignItems:"flex-start",gap:8,padding:"6px 0",
                  ...
                  <div style={{display:"flex",gap:3,flexShrink:0}}>
                    {/* 🕐 Изменить время — для ВСЕХ событий */}
                    <div className="ico-btn" style={{color:T.gold,opacity:.7,fontSize:12}} title="Изменить время"
                      onClick={()=>{
                        ...
                        else if(ev.id==="wake"){if(setProfile)setProfile(p=>({...p,wake:padded}));}
                        else if(ev.id==="sleep"){if(setProfile)setProfile(p=>({...p,sleep:padded}));}
                        else if(ev.id==="commute-to") setCommuteSettings(p=>({...p,toWorkTime:padded}));
                        else if(ev.id==="commute-from") setCommuteSettings(p=>({...p,fromWorkTime:padded}));
                        else if(ev.id&&ev.id.startsWith("pet-feed-")) setFeedTimesOverride(p=>({...p,[parseInt(ev.id.replace("pet-feed-",""))]:padded}));
                        else if(ev.type==="beauty"&&ev.beautyGroup) ev.beautyGroup.forEach(id=>setTasks(p=>p.map(t=>t.id===id?{...t,preferredTime:padded}:t)));
                        notify("Время обновлено ✦");
                      }}>🕐</div>
                    {/* ✏️ Изменить название — для ВСЕХ не-якорных событий */}
                    {ev.type!=="anchor"&&ev.type!=="commute"&&(
                      <div className="ico-btn" style={{color:T.teal,opacity:.7,fontSize:12}} title="Изменить название"
                        onClick={()=>{
                          const newTitle = window.prompt("Новое название:", ev.title);
                          if(!newTitle||!newTitle.trim()) return;
                          if(ev.taskId) setTasks(p=>p.map(t=>t.id===ev.taskId?{...t,title:newTitle.trim()}:t));
                          else if(ev.type==="beauty"&&ev.beautyGroup&&ev.beautyGroup.length===1)
                            setTasks(p=>p.map(t=>t.id===ev.beautyGroup[0]?{...t,title:newTitle.trim()}:t));
                          notify("Название обновлено ✦");
                        }}>✏️</div>
                    )}
                    {/* ↻ Перенести — только для задач */}
                    {!ev.fixed&&ev.taskId&&(
                      <div className="ico-btn danger" style={{fontSize:12}} title="Перенести"
...
function ScheduleSection({profile,tasks,setTasks,today,kb,notify}) {
  ...
  const getDayTasks = (d) => {
    ...
    if(beautyDay.length>0){
      ...
      groups.forEach((grp,gi)=>{
        const isRareSingle=grp.length===1&&(()=>{const m=(grp[0].freq||"").match(/every:(\d+)/);const n=parseInt(m?m[1]:0);return n>=14;})();
        const totalDur=grp.reduce((s,t)=>s+(t.beautyDuration||15),0);
              ...
              {[...anchors.map(a=>({...a,isAnchor:true})),
                ...
                return(
                  <div key={i} style={{display:"flex",gap:8,padding:"6px 0",borderBottom:"1px solid "+T.bdrS}}>
                    ...
                    {/* Чекбокс выполнено — у ВСЕХ событий */}
                    {ev.type!=="anchor"&&(
                      <div onClick={e=>{e.stopPropagation();
                        if(ev.task){
                          setTasks(p=>p.map(t=>t.id===ev.task.id?{...t,doneDate:t.doneDate===dStr?null:dStr,lastDone:t.doneDate===dStr?t.lastDone:dStr}:t));
                        } else if(ev.isBeautyGroup&&ev.beautyGroup){
                          const allDone=ev.beautyGroup.every(id=>tasks.find(t=>t.id===id)?.doneDate===dStr);
                          ev.beautyGroup.forEach(id=>setTasks(p=>p.map(t=>t.id===id?{...t,doneDate:allDone?null:dStr,lastDone:allDone?t.lastDone:dStr}:t)));
                        }
                      }} style={{width:18,height:18,borderRadius:4,flexShrink:0,cursor:"pointer",
                        border:"1.5px solid "+((ev.task?.doneDate===dStr||ev.doneDate===dStr)?T.success:T.bdr),
                        background:(ev.task?.doneDate===dStr||ev.doneDate===dStr)?"rgba(45,106,79,0.2)":"transparent",
                        display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,color:T.success}}>
                        {(ev.task?.doneDate===dStr||ev.doneDate===dStr)?"✓":""}
                      </div>
                    )}
                    <span style={{fontSize:13,color:ev.task?(ev.task.doneDate===dStr?T.text3:T.text1):typeColor[ev.type]||T.text2,
                      textDecoration:(ev.task?.doneDate===dStr||ev.doneDate===dStr)?"line-through":"none",flex:1}}>
                      {ev.label}
                    </span>
                    {/* 🕐 ✏️ у всех событий */}
                    <div style={{display:"flex",gap:3,flexShrink:0}}>
                      <div className="ico-btn" style={{fontSize:11,color:T.gold,padding:"1px 4px"}} title="Изменить время"
                        onClick={e=>{e.stopPropagation();
                          ...
                          if(ev.task) setTasks(p=>p.map(x=>x.id===ev.task.id?{...x,preferredTime:t}:x));
                          else if(ev.isBeautyGroup&&ev.beautyGroup) ev.beautyGroup.forEach(id=>setTasks(p=>p.map(x=>x.id===id?{...x,preferredTime:t}:x)));
                        }}>🕐</div>
                      {ev.task&&<div className="ico-btn" style={{fontSize:11,color:T.teal,padding:"1px 4px"}} title="Изменить название"
                        onClick={e=>{e.stopPropagation();
...
function WorkSection({profile,tasks,setTasks,today,kb,notify}) {
  ...
  const [weekOpen,     setWeekOpen]     = useStorage("ld_work_open_week",     true);
  const [upcomingOpen, setUpcomingOpen] = useStorage("ld_work_open_upcoming",  true);
  const [groupsOpen,   setGroupsOpen]   = useStorage("ld_work_open_groups",    true);
  const [tasksOpen,    setTasksOpen]    = useStorage("ld_work_open_tasks",     true);
  const [adviceOpen,   setAdviceOpen]   = useStorage("ld_work_open_advice",    true);
  ...
  return(
    <div>
      ...
      {(upcoming.length>0||overdue.length>0)&&(
        <div style={{marginBottom:12}}>
          <div onClick={()=>setUpcomingOpen(o=>!o)}
            style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",padding:"8px 12px",borderRadius:upcomingOpen?"10px 10px 0 0":"10px",background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.07)",marginBottom:0,transition:"border-radius .15s"}}>
            <div style={{fontSize:10,color:T.text3,fontFamily:"'JetBrains Mono'",letterSpacing:1.5,flex:1}}>БЛИЖАЙШИЕ ОТЧЁТЫ (7 ДНЕЙ)</div>
            {overdue.length>0&&<span style={{fontSize:10,color:T.danger,fontFamily:"'JetBrains Mono'",background:"rgba(232,85,109,0.12)",padding:"1px 6px",borderRadius:6}}>⚠ {overdue.length}</span>}
            {upcoming.length>0&&<span style={{fontSize:10,color:T.gold,fontFamily:"'JetBrains Mono'",background:"rgba(200,164,90,0.1)",padding:"1px 6px",borderRadius:6}}>{upcoming.length}</span>}
            <span style={{fontSize:11,color:T.text3}}>{upcomingOpen?"▲":"▼"}</span>
          </div>
          {upcomingOpen&&(
            <div className="card" style={{padding:"4px 14px",borderRadius:"0 0 10px 10px",borderTop:"none"}}>
              {overdue.length>0&&overdue.map(r=><ReportRow key={r.id} r={r} showGroup={true}/>)}
              {upcoming.map(r=><ReportRow key={r.id} r={r} showGroup={true}/>)}
              {upcoming.length===0&&overdue.length===0&&(
                <div style={{fontSize:14,color:T.text3,fontStyle:"italic",padding:"8px 0"}}>Нет отчётов в ближайшие 7 дней</div>
              )}
            </div>
          )}
        </div>
...
function ProcForm({item, procSettings, setProcSettings, confirmProc, DAY_RECS, isRare, notify, onClose}) {
  ...
  const needDate = isRare(item.freq);  // every:14+
  const isDaily = item.freq==="daily";
  const n = parseInt((item.freq||"").split(":")[1]||0);
  const isSliding = item.freq && item.freq.startsWith("every:") && n > 0 && n < 14; // every:2,3,7
  ...
  const [formStartDate, setFormStartDate] = useState(existing.startDate||new Date().toISOString().split("T")[0]);
  ...
  return (
    <div style={{margin:"8px 0 4px 0",padding:"12px 14px",background:"rgba(200,164,90,0.06)",borderRadius:12,border:"1px solid rgba(200,164,90,0.2)"}}>
      ...
      {/* Скользящие (every:2,3,7) — стартовая дата */}
      {isSliding&&(
        <div style={{marginBottom:10}}>
          <div style={{fontSize:11,color:T.text3,marginBottom:4}}>
            Начать с даты
            <span style={{color:T.gold,marginLeft:4}}>· далее каждые {n} дн. автоматически</span>
          </div>
          <input type="date" value={formStartDate} onChange={e=>setFormStartDate(e.target.value)}
            style={{padding:"6px 10px",borderRadius:8,border:"1px solid "+T.bdr,background:"rgba(255,255,255,0.04)",color:T.text0,fontSize:14,outline:"none"}}/>
          <div style={{fontSize:10,color:T.text3,marginTop:3,fontStyle:"italic"}}>{rec||"Выбери удобную стартовую дату"}</div>
        </div>
      )}
      {/* День недели — только для еженедельных (every:7) */}
      {!isDaily&&!needDate&&!isSliding&&(
        <div style={{marginBottom:10}}>
          <div style={{fontSize:11,color:T.text3,marginBottom:4}}>
...
function BeautySection({profile,tasks,setTasks,today,kb,notify}) {
  ...
  // Активные инлайн-формы — Set id процедур (Вариант Б: независимые)
  const [activeForms, setActiveForms] = useState({});
  const toggleForm = (id) => setActiveForms(p=>({...p,[id]:!p[id]}));
  const closeForm  = (id) => setActiveForms(p=>({...p,[id]:false}));
  const moon = getMoon();
          ...
          {cat.items.map(item=>{
            ...
            return (
              <div key={item.id}>
                ...
                <div style={{display:"inline-flex",alignItems:"center",gap:6,padding:"6px 12px",borderRadius:20,cursor:"pointer",marginBottom:isActive?0:6,marginRight:6,
                  ...
                  <div>
                    ...
                    <div style={{fontSize:10,color:T.text3,fontFamily:"'JetBrains Mono'"}}>
                      ...
                      {confirmed&&s.day&&<span style={{color:T.success,marginLeft:4}}>· {s.day}</span>}
                      {confirmed&&s.date&&!s.day&&<span style={{color:T.success,marginLeft:4}}>· {s.date}</span>}
                      {confirmed&&s.startDate&&!s.date&&!s.day&&<span style={{color:T.success,marginLeft:4}}>· каждые {parseInt((item.freq||"").split(":")[1]||0)} дн. с {s.startDate}</span>}
                    </div>
                  </div>
                ...
                {/* Инлайн-форма */}
                {isActive&&<ProcForm item={item} procSettings={procSettings} setProcSettings={setProcSettings} confirmProc={confirmProc} DAY_RECS={DAY_RECS} isRare={isRare} notify={notify} onClose={()=>closeForm(item.id)}/>}
              </div>
            );
          })}
        </div>
