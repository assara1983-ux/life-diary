      ...
      <div className="ob-card">
        ...
        {s.id==="persona2" && <>
          {[
            ...
            ["Как восстанавливаешься?","recovery",["Сон и тишина","Прогулка на природе","Общение с близкими","Любимое хобби","Спорт и движение","Уход за собой","Вкусная еда","Кино / книга","Музыка","Медитация","Горячая ванна","Время в одиночестве"],true],
          ].map(([label,key,multi])=>(
            <div className="fld" key={key}>
              <label>{label}</label>
...
function TodaySection({profile,tasks,setTasks,journal,setJournal,today,moon,kb,notify,petLog,setPetLog}) {
  ...
  // Умный фильтр "сегодня": не показывать долгие задачи которые никогда не выполнялись
  // Показываем: ежедневные, конкретные дни недели, конкретные числа, повторные задачи
  const isTrulyDue = (t) => {
...
function getProfDeadlines(profile) {
  ...
  const now = new Date();
  const year = now.getFullYear(); // текущий год (2026)
  const tasks = [];

  // mk создаёт дедлайн ВСЕГДА в текущем году — не сдвигает в будущее
  // Прошедшие дедлайны попадают в "Просроченные", будущие — в "Ближайшие"
  ...
  //  НК РК: ст.149 (КПН), ст.161 (ИПН), ст.209 (ФНО 200),
  //  ст.314 (НДС), ст.424 (ОСМС), ст.484 (ОПВ),
  //  ст.697 (имущество), ст.701 (земля), ст.704 (транспорт)
  //  Все даты проверены для 2026 года
  // ════════════════════════════════════════════════════
  if(prof.includes("Бухгалтер") || prof.includes("ИП")) {

    // ══ ЯНВАРЬ 2026 ══════════════════════════════════
    tasks.push(mk("🏛 КГД: ФНО 911.00 — Патент (продление на 2026 г.)",   1,  5, "Подать до начала деятельности. Уплата ИПН 1% + соцналог — одновременно с подачей","КГД"));
    tasks.push(mk("📊 БНС: Форма 1-Т (4 кв. 2025)",                       1, 15, "Численность и з/п за 4 квартал 2025 года","БНС"));
    tasks.push(mk("📊 БНС: Форма ОО-1 — Образование (год 2025)",          1, 20, "Годовой статотчёт для образовательных организаций за 2025","БНС"));
    tasks.push(mk("🏥 МЗ: Форма №14 — Стационар (год 2025)",              1, 25, "Годовой отчёт стационара в Минздрав за 2025","МЗ"));
    tasks.push(mk("🏥 МЗ: Форма №30 — Амбулатория (год 2025)",            1, 25, "Годовой отчёт поликлиники в Минздрав за 2025","МЗ"));
    tasks.push(mk("🏛 КГД: ФНО 200.00 — Зарплатный (4 кв. 2025)",        1, 25, "ИПН + ОПВ + ВОСМС + ООСМС + СО за октябрь-декабрь 2025. УПЛАТА тех же налогов — до 25 января","КГД"));
    tasks.push(mk("🏛 КГД: ФНО 300.00 — НДС (4 кв. 2025)",               1, 25, "Декларация по НДС за 4 квартал 2025. УПЛАТА НДС — до 25 января","КГД"));
    tasks.push(mk("🏛 КГД: ФНО 910.00 — Упрощённая декл. (2 п/г 2025)",  1, 25, "СНР: декларация за июль-декабрь 2025. УПЛАТА ИПН/КПН + соцналог — до 25 января","КГД"));
    tasks.push(mk("🏥 ФСМС: ОСМС взносы работодателя (4 кв. 2025)",      1, 25, "3% от ФОТ за октябрь-декабрь 2025. Уплата до 25 января","ФСМС"));
    tasks.push(mk("📋 Архивирование кадровых документов за 2025 год",     1, 31, "Сдать трудовые договоры и кадровые приказы за 2025 г. в архив","HR"));

    // ══ ФЕВРАЛЬ 2026 ═════════════════════════════════
    tasks.push(mk("📊 БНС: Форма 4-Т — Условия труда (год 2025)",         2, 10, "Годовой отчёт об условиях труда за 2025 год","БНС"));
    tasks.push(mk("📊 БНС: Форма 1-П — Промышленность (январь 2026)",     2, 10, "Ежемесячный отчёт за январь 2026 (для пром. предприятий)","БНС"));
    tasks.push(mk("📊 БНС: Форма 2-Т — Трудоустройство (год 2025)",       2, 15, "Движение рабочей силы за 2025 год","БНС"));
    tasks.push(mk("📊 МФ: Отчёт об исполнении бюджета (год 2025)",        2, 15, "Годовой бюджетный отчёт в МФ РК за 2025 год","МФ"));
    tasks.push(mk("🔍 КФМ: Отчёт по финансовому мониторингу (год 2025)",  2, 15, "Комитет финмониторинга — для обязанных субъектов","КФМ"));
    tasks.push(mk("🏦 ЕНПФ: Сверка ОПВ за 2025 год",                      2, 28, "Ежегодная сверка обязательных пенсионных взносов с ЕНПФ","ЕНПФ"));
    tasks.push(mk("🏥 ФСМС: Сверка ОСМС за 2025 год",                     2, 28, "Ежегодная сверка взносов на медстрахование","ФСМС"));
    tasks.push(mk("💰 КГД: 1-й авансовый платёж — налог на имущество/землю", 2, 25, "1/4 от суммы налога за 2025 г. Ст.512, 523 НК РК","КГД"));

    // ══ МАРТ 2026 ════════════════════════════════════
    tasks.push(mk("🌿 МЭПР: Экологическая отчётность (год 2025)",         3, 20, "Отчёт в Министерство экологии при наличии природопользования","МЭПР"));
    tasks.push(mk("📊 БНС: Форма 1-ПФ — Финансы предприятий (год 2025)",  3, 15, "Годовой финансовый отчёт в статистику (баланс + ОПУ)","БНС"));
    tasks.push(mk("🏛 КГД: ФНО 100.00 — КПН (год 2025)",                  3, 31, "Корпоративный подоходный налог за 2025 год. Ст.315 НК РК. УПЛАТА — до 10 апреля 2026","КГД"));
    tasks.push(mk("🏛 КГД: ФНО 220.00 — ИПН (год 2025, ОУР)",            3, 31, "ИП на общеустановленном режиме — декларация по ИПН за 2025. Ст.178 НК РК. УПЛАТА — до 10 апреля","КГД"));
    tasks.push(mk("🏛 КГД: ФНО 240.00 — ИПН физлица (год 2025)",         3, 31, "Физлица с имущественным доходом за 2025. Ст.179 НК РК. УПЛАТА — до 10 апреля","КГД"));
    tasks.push(mk("🏛 КГД: ФНО 700.00 — Налог на имущество/землю (2025)", 3, 31, "Годовая декларация за 2025 год. Ст.524 НК РК. УПЛАТА — до 1 апреля 2026","КГД"));
    tasks.push(mk("🏛 КГД: ФНО 870.00 — Транспортный налог (год 2025)",   3, 31, "Декларация ЮЛ за 2025 год. УПЛАТА — до 1 апреля 2026","КГД"));
    tasks.push(mk("🏦 ЕНПФ: ОППВ — проф. взносы работодателя (год 2025)", 3, 31, "При наличии работников на вредных/опасных производствах","ЕНПФ"));
    tasks.push(mk("🔍 АФН: Годовая финансовая отчётность (2025)",         3, 31, "Для финансовых организаций в Агентство финансового надзора","АФН"));
    tasks.push(mk("🏛 АДГС: Декларация о доходах госслужащего (2025)",    3, 31, "Ст.660 НК РК — ежегодно до 31 марта","АДГС"));

    // ══ АПРЕЛЬ 2026 ══════════════════════════════════
    tasks.push(mk("💰 КГД: УПЛАТА налога на имущество/землю/транспорт",   4,  1, "Срок уплаты по годовым декларациям ФНО 700, 870 за 2025 год","КГД"));
    tasks.push(mk("💰 КГД: УПЛАТА КПН/ИПН по годовым декларациям",       4, 10, "Срок уплаты по ФНО 100.00, 220.00, 240.00 за 2025 год. Ст.149, 178 НК РК","КГД"));
    tasks.push(mk("📊 БНС: Форма ДАП-ПМ (1 кв. 2026)",                   4,  8, "Деловая активность малого бизнеса промышленности за 1 кв. 2026","БНС"));
    tasks.push(mk("📊 БНС: Форма 1-П — Промышленность (апрель 2026)",     4, 10, "Ежемесячный отчёт за апрель 2026","БНС"));
    tasks.push(mk("📊 БНС: Форма 1-Т (1 кв. 2026) — Труд и зарплата",    4, 10, "Отчёт за 1 квартал 2026","БНС"));
    tasks.push(mk("📊 БНС: Форма 11 — Основные средства (год 2025)",      4, 15, "Годовой отчёт об основных фондах за 2025","БНС"));
    tasks.push(mk("📊 БНС: Форма 1-МП — Малый бизнес (год 2025)",         4, 15, "Годовой отчёт МСБ за 2025 год","БНС"));
    tasks.push(mk("🏛 МЮ: Годовой отчёт НКО (2025)",                      4, 30, "Некоммерческие организации — отчёт в Министерство юстиции","МЮ"));
    tasks.push(mk("🔍 АПК: Антикоррупционное уведомление",                4, 30, "Агентство по противодействию коррупции — ежегодно","АПК"));
    tasks.push(mk("🏛 КГД: ФНО 200.00 — Зарплатный (1 кв. 2026)",        4, 25, "ИПН + ОПВ + ВОСМС + ООСМС + СО за январь-март 2026. УПЛАТА — до 25 апреля","КГД"));
    tasks.push(mk("🏛 КГД: ФНО 300.00 — НДС (1 кв. 2026)",               4, 25, "Декларация по НДС за 1 квартал 2026. УПЛАТА НДС — до 25 апреля","КГД"));
    tasks.push(mk("🏥 ФСМС: ОСМС взносы работодателя (1 кв. 2026)",      4, 25, "3% от ФОТ за январь-март 2026. Уплата до 25 апреля","ФСМС"));
    tasks.push(mk("💰 КГД: Аванс по КПН — апрель 2026",                   4, 25, "Ежемесячный авансовый платёж по КПН. Ст.305 НК РК","КГД"));

    // ══ МАЙ 2026 ═════════════════════════════════════
    tasks.push(mk("💰 КГД: 2-й авансовый платёж — налог на имущество/землю", 5, 25, "1/4 от суммы налога. Ст.512, 523 НК РК","КГД"));
    tasks.push(mk("💰 КГД: Аванс по КПН — май 2026",                      5, 25, "Ежемесячный авансовый платёж","КГД"));
    tasks.push(mk("🏛 КГД: ФНО 150.00 — Трансфертное ценообразование",    5, 31, "При наличии контролируемых сделок с нерезидентами. Ст.295 НК РК","КГД"));

    // ══ ИЮНЬ 2026 ════════════════════════════════════
    tasks.push(mk("📋 Годовое собрание ТОО / акционеров АО",              6, 30, "Не позднее 6 месяцев после окончания финансового года. ЗРК «О ТОО» ст.45","МЮ"));
    tasks.push(mk("💰 КГД: Аванс по КПН — июнь 2026",                     6, 25, "Ежемесячный авансовый платёж","КГД"));

    // ══ ИЮЛЬ 2026 ════════════════════════════════════
    tasks.push(mk("📊 БНС: Форма 1-П — Промышленность (июль 2026)",       7, 10, "Ежемесячный отчёт за июль 2026","БНС"));
    tasks.push(mk("📊 БНС: Форма 1-Т (2 кв. 2026) — Труд и зарплата",    7, 10, "Отчёт за 2 квартал 2026","БНС"));
    tasks.push(mk("🏛 КГД: ФНО 200.00 — Зарплатный (2 кв. 2026)",        7, 25, "ИПН + ОПВ + ВОСМС + ООСМС + СО за апрель-июнь 2026. УПЛАТА — до 25 июля","КГД"));
    tasks.push(mk("🏛 КГД: ФНО 300.00 — НДС (2 кв. 2026)",               7, 25, "Декларация по НДС за 2 квартал 2026. УПЛАТА НДС — до 25 июля","КГД"));
    tasks.push(mk("🏥 ФСМС: ОСМС взносы работодателя (2 кв. 2026)",      7, 25, "3% от ФОТ за апрель-июнь 2026. Уплата до 25 июля","ФСМС"));
    tasks.push(mk("💰 КГД: Аванс по КПН — июль 2026",                     7, 25, "Ежемесячный авансовый платёж","КГД"));

    // ══ АВГУСТ 2026 ══════════════════════════════════
    tasks.push(mk("📊 БНС: Форма 1-П — Промышленность (август 2026)",     8, 10, "Ежемесячный отчёт за август 2026","БНС"));
    tasks.push(mk("🏛 КГД: ФНО 910.00 — Упрощённая декл. (1 п/г 2026)",  8, 15, "СНР: декларация за январь-июнь 2026. Ст.688 НК РК","КГД"));
    tasks.push(mk("💰 КГД: 3-й авансовый платёж — налог на имущество/землю", 8, 25, "1/4 от суммы налога. Ст.512, 523 НК РК","КГД"));
    tasks.push(mk("💰 КГД: УПЛАТА по ФНО 910.00 (1 п/г 2026)",           8, 25, "Уплата ИПН/КПН + соцналог по упрощённой декларации за 1-е полугодие","КГД"));
    tasks.push(mk("💰 КГД: Аванс по КПН — август 2026",                   8, 25, "Ежемесячный авансовый платёж","КГД"));

    // ══ СЕНТЯБРЬ 2026 ════════════════════════════════
    tasks.push(mk("📋 Воинский учёт — годовая сверка",                     9,  1, "Ежегодная сверка списков с местным военкоматом (ВЦУ)","HR"));
    tasks.push(mk("💰 КГД: Аванс по КПН — сентябрь 2026",                 9, 25, "Ежемесячный авансовый платёж","КГД"));

    // ══ ОКТЯБРЬ 2026 ═════════════════════════════════
    tasks.push(mk("📊 БНС: Форма 1-П — Промышленность (октябрь 2026)",   10, 10, "Ежемесячный отчёт за октябрь 2026","БНС"));
    tasks.push(mk("📊 БНС: Форма 1-Т (3 кв. 2026) — Труд и зарплата",   10, 10, "Отчёт за 3 квартал 2026","БНС"));
    tasks.push(mk("🏛 КГД: ФНО 200.00 — Зарплатный (3 кв. 2026)",       10, 25, "ИПН + ОПВ + ВОСМС + ООСМС + СО за июль-сентябрь 2026. УПЛАТА — до 25 октября","КГД"));
    tasks.push(mk("🏛 КГД: ФНО 300.00 — НДС (3 кв. 2026)",              10, 25, "Декларация по НДС за 3 квартал 2026. УПЛАТА НДС — до 25 октября","КГД"));
    tasks.push(mk("🏥 ФСМС: ОСМС взносы работодателя (3 кв. 2026)",     10, 25, "3% от ФОТ за июль-сентябрь 2026. Уплата до 25 октября","ФСМС"));
    tasks.push(mk("💰 КГД: Аванс по КПН — октябрь 2026",                 10, 25, "Ежемесячный авансовый платёж","КГД"));

    // ══ НОЯБРЬ 2026 ══════════════════════════════════
    tasks.push(mk("📊 БНС: Форма 1-П — Промышленность (ноябрь 2026)",    11, 10, "Ежемесячный отчёт за ноябрь 2026","БНС"));
    tasks.push(mk("💰 КГД: 4-й авансовый платёж — налог на имущество/землю", 11, 25, "Последний авансовый платёж. Ст.512, 523 НК РК","КГД"));
    tasks.push(mk("💰 КГД: Аванс по КПН — ноябрь 2026",                  11, 25, "Ежемесячный авансовый платёж","КГД"));

    // ══ ДЕКАБРЬ 2026 ═════════════════════════════════
    tasks.push(mk("🏛 КГД: Проверка и продление лицензий",               12,  1, "Проверить сроки действия всех лицензий заблаговременно","КГД"));
    tasks.push(mk("📋 График отпусков на 2027 год",                       12, 15, "Утвердить до 15 декабря включительно. ТК РК ст.93","HR"));
    tasks.push(mk("💰 КГД: Аванс по КПН — декабрь 2026",                 12, 25, "Ежемесячный авансовый платёж","КГД"));
  }
  ...
  if(prof.includes("HR") || prof.includes("Кадры")) {
    tasks.push(mk("📊 БНС: Форма 1-Т (4 кв.) — Труд и зарплата",     1, 15, "Отчёт за 4 квартал прошлого года","БНС"));
    tasks.push(mk("📋 Сдача трудовых договоров в архив (год)",        1, 31, "Документы за прошлый год передать в архив","HR"));
    ...
    tasks.push(mk("🏦 ЕНПФ: Сверка ОПВ (год)",                        2, 28, "Ежегодная сверка с ЕНПФ по пенсионным взносам","ЕНПФ"));
    tasks.push(mk("🏥 ФСМС: Сверка ОСМС (год)",                       2, 28, "Сверка взносов на медстрахование за год","ФСМС"));
    tasks.push(mk("🏛 КГД: ФНО 200.00 — Зарплатный (1 кв.)",          4, 25, "ИПН, ОПВ, ОСМС, СО — 1 квартал. Уплата до 25 апреля","КГД"));
    tasks.push(mk("📊 БНС: Форма 1-Т (1 кв.) — Труд и зарплата",      4, 10, "Отчёт за 1 квартал","БНС"));
    tasks.push(mk("🏛 КГД: ФНО 200.00 — Зарплатный (2 кв.)",          7, 25, "Уплата до 25 июля","КГД"));
    tasks.push(mk("📊 БНС: Форма 1-Т (2 кв.) — Труд и зарплата",      7, 10, "Отчёт за 2 квартал","БНС"));
    tasks.push(mk("📋 Воинский учёт — годовая сверка",                 9,  1, "Сверка с военкоматом (ВЦУ)","HR"));
    tasks.push(mk("🏛 КГД: ФНО 200.00 — Зарплатный (3 кв.)",         10, 25, "Уплата до 25 октября","КГД"));
    tasks.push(mk("📊 БНС: Форма 1-Т (3 кв.) — Труд и зарплата",     10, 10, "Отчёт за 3 квартал","БНС"));
    tasks.push(mk("📋 График отпусков на следующий год",               12, 15, "Утвердить до 15 декабря","HR"));
    tasks.push(mk("🏛 КГД: ФНО 200.00 — Зарплатный (4 кв.)",          1, 25, "Уплата до 25 января следующего года","КГД"));
  }
  ...
  if(prof.includes("Юрист")) {
    tasks.push(mk("📋 Подготовка годового отчёта ТОО/АО",              3, 31, "Подготовить финансовую отчётность для годового собрания","МЮ"));
    tasks.push(mk("🏛 МЮ: Перерегистрация при изменениях устава",      1, 31, "В течение 30 дней с момента изменений","МЮ"));
    tasks.push(mk("🔍 АФН: Годовая финансовая отчётность",             3, 31, "Для финансовых организаций — в Агентство финнадзора","АФН"));
    tasks.push(mk("💰 КГД: Уплата налогов ЮЛ по годовым декларациям", 4, 10, "Срок уплаты КПН, ИПН, налога на имущество","КГД"));
    tasks.push(mk("📋 Годовое собрание ТОО / акционеров АО",           6, 30, "Не позднее 6 месяцев после окончания финансового года","МЮ"));
    tasks.push(mk("🌿 МЭПР: Экологическое разрешение/отчёт",           3, 20, "При наличии природопользования","МЭПР"));
    tasks.push(mk("🏛 КГД: Проверка и продление лицензий",            12,  1, "Проверить сроки действия всех лицензий заблаговременно","КГД"));
  }
  ...
  if(prof.includes("Врач") || prof.includes("Мед")) {
    tasks.push(mk("🔍 Проверка лицензии медицинской деятельности",    1, 15, "Проверить срок действия лицензии","МЗ"));
    tasks.push(mk("🏥 МЗ: Форма № 14 — Стационар (год)",             1, 25, "Годовой отчёт стационара в Минздрав","МЗ"));
    tasks.push(mk("🏥 МЗ: Форма № 30 — Амбулатория (год)",           1, 25, "Годовой отчёт поликлиники в Минздрав","МЗ"));
    tasks.push(mk("💊 Отчёт по обороту наркотических средств (1 кв.)",4, 10, "В Комитет фармконтроля — ежеквартально","КФК"));
    tasks.push(mk("🏥 МЗ: Медстатистика (2 кв.)",                    7, 10, "Квартальный отчёт в Минздрав","МЗ"));
    tasks.push(mk("💊 Отчёт по обороту наркотических средств (2 кв.)",7, 10, "В Комитет фармконтроля","КФК"));
    tasks.push(mk("💊 Отчёт по обороту наркотических средств (3 кв.)",10, 10, "В Комитет фармконтроля","КФК"));
    tasks.push(mk("🏥 МЗ: Медстатистика (3 кв.)",                   10, 10, "Квартальный отчёт в Минздрав","МЗ"));
    tasks.push(mk("📋 КККМФД: Сертификация специалиста (плановая)",   9,  1, "Раз в 5 лет — подача документов в аттестационную комиссию","КККМФД"));
    tasks.push(mk("💊 Отчёт по обороту наркотических средств (год)", 1, 15, "Годовой отчёт в Комитет фармконтроля","КФК"));
  }
  ...
  if(prof.includes("Педагог")) {
    tasks.push(mk("📊 БНС: Форма ОО-1 — Образовательная организация", 1, 20, "Годовой статотчёт по образованию","БНС"));
    tasks.push(mk("📚 МОН: Итоговый отчёт 2-я четверть",              1, 15, "Отчёт по итогам 2-й четверти","МОН"));
    tasks.push(mk("📋 Аттестация педагога (плановая)",                 3,  1, "Раз в 5 лет — подача документов в аттестационную комиссию","МОН"));
    tasks.push(mk("📚 МОН: Итоговый отчёт 3-я четверть",              4,  5, "Отчёт по итогам 3-й четверти","МОН"));
    tasks.push(mk("📚 МОН: Самоанализ за учебный год",                 6,  1, "Годовой самоанализ педагога","МОН"));
    tasks.push(mk("📚 МОН: Рабочие программы на учебный год",          9,  1, "Сдать до начала учебного года","МОН"));
    tasks.push(mk("📚 МОН: Итоговый отчёт 1-я четверть",             11,  5, "Отчёт по итогам 1-й четверти","МОН"));
    tasks.push(mk("📚 МОН: Итоговый отчёт 4-я четверть",              6, 15, "Финальный отчёт учебного года","МОН"));
  }
  ...
  if(prof.includes("Госслужащий")) {
    tasks.push(mk("📊 МФ: Отчёт об исполнении бюджета (год)",          2, 15, "Годовой отчёт в Министерство финансов","МФ"));
    tasks.push(mk("🏛 АДГС: Декларация о доходах и имуществе (год)",   3, 31, "Ежегодная декларация — Агентство по делам госслужбы","АДГС"));
    tasks.push(mk("📊 МФ: Отчёт об исполнении бюджета (1 кв.)",        4, 15, "Квартальный отчёт в Министерство финансов","МФ"));
    tasks.push(mk("🔍 АПК: Антикоррупционное уведомление",             4, 30, "Ежегодное уведомление об отсутствии конфликта интересов","АПК"));
    tasks.push(mk("📊 МФ: Отчёт об исполнении бюджета (2 кв.)",        7, 15, "Квартальный отчёт","МФ"));
    tasks.push(mk("📊 МФ: Отчёт об исполнении бюджета (3 кв.)",       10, 15, "Квартальный отчёт","МФ"));
  }
