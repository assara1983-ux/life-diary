...
@media(max-width:380px) {
  html { font-size:14px; }
  .sidebar { width:48px; }
  .sidebar-item { padding:8px 4px; font-size:9px; }
  .sidebar-emoji { font-size:18px; }
  .main { margin-left:48px; }
  .page { padding:10px; }
  .card { padding:14px; margin-bottom:10px; }
  .ai-box { padding:14px; }
  .ai-list-item { padding:10px 12px; gap:10px; }
  .ai-list-num { width:24px; height:24px; font-size:11px; }
  .ai-list-text, .ai-list-title, .ai-paragraph { font-size:14px; }
  .ai-header { font-size:16px; }
  .btn-sm { padding:6px 10px; font-size:12px; }
  .hdr { padding:10px 12px; }
  .hdr-title { font-size:18px; }
}

/* Стандартные телефоны (380–700px) */
@media(min-width:381px) and (max-width:700px) {
  ...
  .ai-list-item { padding:12px 14px; }
  .ai-header { font-size:17px; }
}
...
@media(min-width:1025px) {
  ...
  .page { padding:32px 40px; max-width:920px; margin:0 auto; }
}

/* Очень большие экраны (1440px+) */
@media(min-width:1440px) {
  .page { max-width:1080px; padding:36px 56px; }
  .ai-box { padding:24px; }
  .ai-list-item { padding:16px 18px; }
}

/* Сенсорные устройства — увеличиваем кнопки для удобства тапа */
@media(hover:none) and (pointer:coarse) {
  .btn, .tab, .chk, .ico-btn { min-height:40px; }
...
export default function LifeDiary() {
  ...
  // Адаптивный viewport для разных устройств
  useEffect(()=>{
    ...
    // Отслеживание ширины для обновления layout
    const updateLayout = () => {
      const w = window.innerWidth;
      document.documentElement.style.setProperty("--screen-w", w + "px");
      if(w < 380) document.documentElement.dataset.screen = "xs";
      else if(w < 700) document.documentElement.dataset.screen = "sm";
      else if(w < 1024) document.documentElement.dataset.screen = "md";
      else document.documentElement.dataset.screen = "lg";
    };
    updateLayout();
...
function AiBox({ kb, prompt, label="ИИ-СОВЕТНИК", btnText="Получить совет", placeholder="Нажми — получи персональный совет...", actionType=null }) {
  // Ключ для хранения текущего ответа этого блока
  const cacheKey = "ld_ai_cur_" + label.replace(/\s+/g,"_");
  ...
  const ask = async()=>{
    setLoading(true);
    // Если уже был ответ — отправляем старый в журнал
    if(text) {
      try {
        const journal = JSON.parse(localStorage.getItem("ld_ai_journal") || "[]");
        journal.unshift({date: new Date().toISOString(), label, text});
        localStorage.setItem("ld_ai_journal", JSON.stringify(journal.slice(0, 200)));
      } catch{}
    }
    const r = await askClaude(kb, prompt);
    setText(r);
    try { localStorage.setItem(cacheKey, r); } catch{}
    setLoading(false);
  };
  ...
  const addToShopping = () => {
    ...
    try {
      ...
      const catRules = [
        {cat:"Бытовая химия",words:["мыло","порошок","гель для","шампунь","ополаск","чистящ","моющ","туалет","губка","перчатк","пятновыв","отбелив","кондиционер для бель"]},
      ...
      const newItems = allListItems.map(t => {
        ...
        let cat = "Продукты";
        const m = t.match(/^\[([^\]]+)\]/);
        if(m) cat = m[1]; else cat = detect(name);
        return {id:Date.now()+Math.random(), name:name.length>80?name.slice(0,77)+"...":name, cat, done:false};
      });
    ...
    <div className="ai-box">
          ...
          if(b.type === "list") return <div key={i} className="ai-list">
            {b.items.map((item, j) => <div key={j} className="ai-list-item">
              ...
              <div className="ai-list-body">
                ...
                <div className="ai-list-text">{typeof item==="string"?item:(item.body || item.title)}</div>
              </div>
            </div>)}
          </div>;
      ...
      <div className="ai-actions">
        ...
        {text && allListItems.length > 0 && <>
          ...
          {actionType === "shopping" && <button className="btn btn-ghost btn-sm" onClick={addToShopping}>🛒 В покупки</button>}
          <button className="btn btn-ghost btn-sm" onClick={addAllToCalendar}>📅 Календарь</button>
        </>}
        {text && <button className="btn btn-ghost btn-sm" onClick={moveToJournal}>📖 В журнал</button>}
      </div>
    </div>
  );
}
...
function JournalSection({journal,setJournal,today,notify}) {
  ...
  return(
    <div>
      <div className="tabs">{[["today","Сегодня"],["all","Все записи"],["ai","AI-ответы"],["stats","Статистика"]].map(([v,l])=><div key={v} className={"tab"+(view===v?" on":"")} onClick={()=>setView(v)}>{l}</div>)}</div>
      {view==="today"&&(
        <div className="card">
      ...
      {view==="ai"&&(()=>{
        try {
          ...
          if(!aiJournal.length) return <div className="empty"><span className="empty-ico">📖</span><p>Журнал AI-ответов пуст. При обновлении совета предыдущий ответ сохранится здесь.</p></div>;
          // Группируем по разделу (label)
          const grouped = {};
          ...
          return <>
            {Object.entries(grouped).map(([lbl, items]) => (
              <div key={lbl} className="card">
                ...
                {items.map((it, i) => (
                  <details key={i} style={{marginBottom:8,padding:"8px 0",borderBottom:"1px solid rgba(255,255,255,.04)"}}>
                    <summary style={{cursor:"pointer",fontSize:13,color:T.goldD,fontFamily:"JetBrains Mono",letterSpacing:1}}>{new Date(it.date).toLocaleDateString("ru-RU",{day:"numeric",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit"})}</summary>
                    <div style={{marginTop:10,padding:12,background:"rgba(255,255,255,.02)",borderRadius:8,fontSize:14,color:T.text1,lineHeight:1.6,whiteSpace:"pre-wrap",fontFamily:"'Crimson Pro',serif"}}>{it.text}</div>
                  </details>
                ))}
              </div>
            ))}
            <button className="btn btn-ghost btn-sm" onClick={()=>{if(confirm("Очистить весь журнал AI?")){localStorage.removeItem("ld_ai_journal");notify("Очищено");setView("today");setTimeout(()=>setView("ai"),50);}}}>Очистить журнал</button>
          </>;
        } catch { return null; }
      })()}
      {view==="stats"&&(
    ...
    <div>
      ...
      {view==="astro"&&(
        <div>
          ...
          <AiBox kb={kb} prompt={"Составь персональный астрологический и нумерологический портрет. Данные для расчёта: знак зодиака — "+z.name+", восточный знак — "+east+", градус судьбы по ФИО — "+(deg||"—")+"°, дата рождения — "+(profile.dob||"—")+", число судьбы по нумерологии — "+(numerologyDestiny(profile.dob)||"—")+", возраст — "+(calcAge(profile.dob)||"—")+" лет. ВАЖНО: обращайся ТОЛЬКО на ты, НИКОГДА не называй по имени, не пиши \"Ирине\", \"для Ирины\" — пиши прямо \"ты\", \"тебе\". Структура ответа со строгими заголовками вида ## Название блока:\n\n## По знаку зодиака "+z.name+"\nКак солнечный знак влияет на твой характер — 4 пункта нумерованным списком.\n\n## По восточному знаку "+east+"\nКак "+east+" формирует подход к жизни — 3 пункта.\n\n## По числу судьбы "+(numerologyDestiny(profile.dob)||"—")+"\nЧто означает это число для жизненного пути — 3 пункта.\n\n## По градусу судьбы "+(deg||"—")+"°\nКакая энергия и предназначение скрыты в градусе — 3 пункта.\n\n## Сильные стороны — синтез\nЧто использовать в жизни — 4 пункта.\n\n## Слабые места и над чем работать\n3 пункта.\n\n## Здоровье — зоны внимания\n3 пункта с учётом особенностей знака.\n\n## Любовь и отношения\n3 пункта.\n\n## Карьера и финансы\n3 пункта.\n\nКаждый пункт нумеруй (1) 2) 3)), пиши конкретно и по делу. НЕ используй markdown bold (**). НЕ обращайся по имени."} label="Персональный астропортрет" btnText="Составить мой портрет" placeholder="Получи свой полный астрологический портрет..."/>
          <AiBox kb={kb} prompt={"Расшифруй жизненный путь по периодам жизни. Дата рождения: "+(profile.dob||"—")+" (год рождения "+(profile.dob?new Date(profile.dob).getFullYear():"—")+"), сейчас "+(calcAge(profile.dob)||"—")+" лет. Знак: "+z.name+", восточный: "+east+", число судьбы: "+(numerologyDestiny(profile.dob)||"—")+". ВАЖНО: обращайся на ты, без имени. Используй ТОЧНЫЕ годы. Структура с заголовками ## где периоды идут СТРОГО по возрастанию возраста:\n\n## Детство ("+(profile.dob?new Date(profile.dob).getFullYear():"")+"–"+(profile.dob?new Date(profile.dob).getFullYear()+12:"")+", 0–12 лет)\nКакая ты была в детстве — 3 пункта нумерованным списком.\n\n## Юность ("+(profile.dob?new Date(profile.dob).getFullYear()+13:"")+"–"+(profile.dob?new Date(profile.dob).getFullYear()+21:"")+", 13–21 год)\nЧто формировало твою личность — 3 пункта.\n\n## Молодость ("+(profile.dob?new Date(profile.dob).getFullYear()+22:"")+"–"+(profile.dob?new Date(profile.dob).getFullYear()+35:"")+", 22–35 лет)\nГлавные уроки — 4 пункта.\n\n## Зрелость ("+(profile.dob?new Date(profile.dob).getFullYear()+36:"")+"–"+(profile.dob?new Date(profile.dob).getFullYear()+50:"")+", 36–50 лет)"+(calcAge(profile.dob)>=36&&calcAge(profile.dob)<=50?" — это ТВОЙ ТЕКУЩИЙ ПЕРИОД":"")+"\nПик реализации — 4 пункта.\n\n## Мудрость ("+(profile.dob?new Date(profile.dob).getFullYear()+51:"")+"+, после 51 года)\nЧто впереди — 3 пункта.\n\nКаждый пункт нумеруй. НЕ используй markdown bold."} label="Жизненный путь по годам" btnText="Расшифровать" placeholder="Покажу твой жизненный путь по этапам..."/>
          <AiBox kb={kb} prompt={"Рассчитай ключевые годы жизни на основе нумерологии и астрологических циклов. Дата рождения "+(profile.dob||"—")+", родилась в "+(profile.dob?new Date(profile.dob).getFullYear():"—")+" году, сейчас "+(calcAge(profile.dob)||"—")+" лет (текущий год — "+(new Date().getFullYear())+"). Личный год сейчас: "+(personalYear(profile.dob, new Date().getFullYear())||"—")+". Знак: "+z.name+", число судьбы: "+(numerologyDestiny(profile.dob)||"—")+". Возвращение Сатурна (~30 лет) было в "+(profile.dob?new Date(profile.dob).getFullYear()+30:"—")+". Циклы Юпитера (12 лет): "+(profile.dob?(new Date(profile.dob).getFullYear()+24)+", "+(new Date(profile.dob).getFullYear()+36)+", "+(new Date(profile.dob).getFullYear()+48):"—")+". ВАЖНО: обращайся на ты, без имени. Годы СТРОГО ПО ПОРЯДКУ от прошлого к будущему. Структура с заголовками ##:\n\n## Прошлые ключевые годы\n3 конкретных года из прошлого по возрастанию (например 2008, затем 2014, затем 2019) — каждый с пояснением что это был за период. Нумерованный список.\n\n## "+new Date().getFullYear()+" год — текущий\nЛичный год "+(personalYear(profile.dob, new Date().getFullYear())||"—")+". Главная тема и 3 события года по кварталам — нумерованным списком.\n\n## "+(new Date().getFullYear()+1)+" год\nЛичный год "+(personalYear(profile.dob, new Date().getFullYear()+1)||"—")+". Тема и 3 события — нумерованным списком.\n\n## "+(new Date().getFullYear()+2)+" год\nЛичный год "+(personalYear(profile.dob, new Date().getFullYear()+2)||"—")+". Тема и 3 события — нумерованным списком.\n\n## "+(new Date().getFullYear()+3)+" год\nЛичный год "+(personalYear(profile.dob, new Date().getFullYear()+3)||"—")+". Тема и 3 события — нумерованным списком.\n\n## Дальняя перспектива (5–10 лет вперёд)\n3 ключевых года и события по возрастанию. Не повторяй предыдущие годы.\n\nНЕ используй markdown bold. Каждый блок строго в своём порядке."} label="Ключевые даты жизни" btnText="Показать мои даты" placeholder="Покажу важные годы и события прошлого и будущего..."/>
        </div>
      )}
      {view==="sections"&&(
        <div className="card">
