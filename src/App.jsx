...
// Нумерология: число судьбы = сумма цифр даты рождения
function numerologyDestiny(dob) {
  if(!dob) return null;
...
// Личный год по нумерологии
function personalYear(dob, year) {
  if(!dob) return null;
  const b = new Date(dob);
  const sum = (b.getDate() + (b.getMonth()+1) + year).toString().split("").reduce((s,d)=>s+parseInt(d),0);
  let r = sum;
  while(r > 9) r = r.toString().split("").reduce((s,d)=>s+parseInt(d),0);
  return r;
}
...
function parseAiResponse(text) {
  if(!text) return [];
  // Очистим markdown bold ** оставляя текст
  const cleanBold = (s) => s.replace(/\*\*(.+?)\*\*/g, "$1").replace(/^\*+|\*+$/g, "").trim();
  ...
  for(let rawLine of lines) {
    ...
    if(mdHeader) {
      flushList(); flushText();
      blocks.push({type:"header", content: cleanBold(mdHeader[1])});
      continue;
    }
    ...
    if(/^\*\*[^*]+\*\*:?\s*$/.test(trimmed)) {
      flushList(); flushText();
      blocks.push({type:"header", content: cleanBold(trimmed).replace(/:$/, "")});
      continue;
    }
    
    // 1**заголовок**: текст или 1. **заголовок**: текст
    const numHeader = trimmed.match(/^(\d+)[.)\s]*\*\*(.+?)\*\*:?\s*(.*)$/);
    if(numHeader) {
      flushText();
      if(!currentList) currentList = {type:"list", items:[]};
      const title = cleanBold(numHeader[2]);
      const rest = numHeader[3].trim();
      currentList.items.push({title, body: cleanBold(rest)});
      continue;
    }
    
    // Обычный список: 1) текст, 1. текст, - текст, • текст, * текст
    const listMatch = trimmed.match(/^(?:\d+[.)]\s+|[-•*]\s+)(.+)$/);
    if(listMatch) {
      flushText();
      if(!currentList) currentList = {type:"list", items:[]};
      const itemText = cleanBold(listMatch[1]);
      // Если в пункте есть : — разделим на title и body
      const colonIdx = itemText.indexOf(":");
      if(colonIdx > 0 && colonIdx < 60) {
        currentList.items.push({title: itemText.slice(0, colonIdx).trim(), body: itemText.slice(colonIdx+1).trim()});
      } else {
        currentList.items.push({title: "", body: itemText});
      }
      continue;
    }
    
    // Короткая строка с двоеточием = заголовок
    if(trimmed.length < 70 && trimmed.endsWith(":") && !trimmed.includes(",")) {
      flushList(); flushText();
      blocks.push({type:"header", content: cleanBold(trimmed.slice(0, -1))});
      continue;
    }
    
    // Обычный абзац
    flushList();
    currentText.push(cleanBold(trimmed));
  }
  flushList();
  flushText();
  return blocks;
...
function AiBox({ kb, prompt, label="ИИ-СОВЕТНИК", btnText="Получить совет", placeholder="Нажми — получи персональный совет...", actionType=null, onAddItems=null }) {
  ...
  const addToShopping = () => {
    ...
    try {
      ...
      // Категории и ключевые слова для распознавания
      const catRules = [
        {cat: "Бытовая химия", words: ["мыло","порошок","гель для","шампунь","ополаскиватель","чистящ","моющ","туалет","губка","перчатк","пятновывод","отбелив","кондиционер для бель"]},
        {cat: "Красота и уход", words: ["крем","маска","сыворотк","тоник","скраб","пилинг","масло для","бальзам","помад","тушь","пудр","ватн","косметик","дезодорант","зубн","паст","нить"]},
        {cat: "Для питомцев", words: ["корм","лоток","наполнитель","когтет","ошейник","поводок","игрушк для","для котов","для собак","для кошек","для попуг","для хомяк"]},
        {cat: "Аптека", words: ["витамин","добавк","омега","магний","мелатонин","пробиотик","антиб","сироп","таблетк","капл","мазь","бинт","пластыр","термометр","леденцы от","спрей от","капли от"]},
        {cat: "Одежда", words: ["футболк","рубашк","брюк","юбк","платье","носки","нижне","пижам","халат","куртк","пальто","туфл","ботинк","кроссовк"]},
      ];
      const detectCat = (name) => {
        const low = name.toLowerCase();
        for(const r of catRules) for(const w of r.words) if(low.includes(w)) return r.cat;
        return "Продукты";
      };
      const newItems = allListItems.map(t => {
        // Убираем явное указание категории если есть [Категория] в начале
        let name = t.replace(/^\[[^\]]+\]\s*/,"").trim();
        let cat = "Продукты";
        const catMatch = t.match(/^\[([^\]]+)\]/);
        if(catMatch) cat = catMatch[1];
        else cat = detectCat(name);
        return {
          id: Date.now() + Math.random(),
      ...
      // Группируем для уведомления
      const byCount = {};
      newItems.forEach(i => { byCount[i.cat] = (byCount[i.cat]||0)+1; });
      const summary = Object.entries(byCount).map(([c,n])=>n+" в "+c).join(", ");
      alert("Добавлено: "+summary);
    } catch(e) { alert("Ошибка сохранения"); }
  };
          ...
          if(b.type === "list") return <div key={i} className="ai-list">
            {b.items.map((item, j) => <div key={j} className="ai-list-item">
              ...
              <div className="ai-list-body">
                {item.title && <div className="ai-list-title">{item.title}</div>}
                <div className="ai-list-text">{item.body || item}</div>
              </div>
            </div>)}
          </div>;
      ...
      <div className="card">
        <div className="card-hd">
          ...
          <div className="btn-row">
            {homeTasks.length>0&&<button className="btn btn-ghost btn-sm" onClick={()=>{const ts=autoHome();setTasks(p=>[...p,...ts]);notify("Добавлено");}}>+ Авто</button>}
            <button className="btn btn-ghost btn-sm" onClick={()=>setModal({})}>+ Своё</button>
          </div>
        </div>
        {due.length===0&&homeTasks.length>0&&<div className="empty"><span className="empty-ico">🏡</span><p>Домашних дел на сегодня нет!</p></div>}
        {due.map(task=>(
          <div key={task.id} className="task-row">
            <div className={`chk${task.doneDate===today?" done":""}`} onClick={()=>setTasks(p=>p.map(t=>t.id===task.id?{...t,doneDate:t.doneDate===today?null:today,lastDone:t.doneDate===today?t.lastDone:today}:t))}>{task.doneDate===today?"✓":""}</div>
            <div className="task-body">
              <div className={`task-name${task.doneDate===today?" done":""}`}>{task.title}</div>
              <div className="task-meta"><span className="badge bt">{freqLabel(task.freq)}</span>{task.lastDone&&<span className="badge bm">был: {task.lastDone}</span>}</div>
            </div>
            <div className="ico-btn danger" onClick={()=>setTasks(p=>p.filter(t=>t.id!==task.id))}>✕</div>
          </div>
        ))}
        {homeTasks.filter(t=>!due.includes(t)).length>0&&(
          <details style={{marginTop:10}}>
            <summary style={{cursor:"pointer",fontSize:13,color:T.text3,padding:"6px 0",fontStyle:"italic"}}>Остальные дела ({homeTasks.filter(t=>!due.includes(t)).length})</summary>
            {homeTasks.filter(t=>!due.includes(t)).map(task=>(
              <div key={task.id} className="task-row" style={{opacity:.45}}>
                <div className="task-body"><div className="task-name">{task.title}</div><div className="task-meta"><span className="badge bm">{freqLabel(task.freq)}</span></div></div>
              </div>
            ))}
          </details>
        )}
          ...
          <div key={cat} style={{marginBottom:16}}>
            ...
            {byCat[cat].map(item=>(
              <div key={item.id} style={{display:"flex",alignItems:"center",gap:10,padding:"7px 0",borderBottom:`1px solid rgba(255,255,255,.04)`}}>
                <div className={`chk${item.done?" done":""}`} onClick={()=>setShopList(p=>p.map(x=>x.id===item.id?{...x,done:!x.done}:x))}>{item.done?"✓":""}</div>
                <span style={{flex:1,fontSize:16,textDecoration:item.done?"line-through":"none",color:item.done?T.text3:T.text0}}>{item.name}</span>
...
function GoalsSection({profile,kb,notify}) {
  ...
  // Wheel of Life — 8 базовых сфер
  const WHEEL_AREAS = ["Здоровье","Карьера","Финансы","Отношения","Семья","Саморазвитие","Творчество","Духовность"];
  const WHEEL_COLORS = ["#7BCCA0","#82AADD","#E5C87A","#E87878","#B882E8","#7EDDD5","#E8A85A","#C8C4BC"];
  const [wheelScores, setWheelScores] = useState(()=>{
    try { return JSON.parse(localStorage.getItem("ld_wheel")||"null") || WHEEL_AREAS.reduce((a,k)=>({...a,[k]:5}),{}); }
    catch { return WHEEL_AREAS.reduce((a,k)=>({...a,[k]:5}),{}); }
  });
  const updateScore = (area, val) => {
    const next = {...wheelScores, [area]:val};
    setWheelScores(next);
    try { localStorage.setItem("ld_wheel", JSON.stringify(next)); } catch{}
  };
  
  // SVG wheel: 8 sectors, each with score from 1-10
  const cx=140, cy=140, maxR=130;
  const wheelPaths = WHEEL_AREAS.map((area, i) => {
    const score = wheelScores[area] || 5;
  ...
  // Labels positioned on outer ring
  const labelPositions = WHEEL_AREAS.map((area, i) => {
    ...
    <div>
      ...
      <div className="card" style={{marginBottom:12}}>
        ...
        <div style={{display:"flex",justifyContent:"center",marginBottom:16}}>
          <svg width="320" height="320" viewBox="-20 -20 320 320" style={{maxWidth:"100%"}}>
            {/* Concentric circles guide */}
            {[2,4,6,8,10].map(n => <circle key={n} cx={cx} cy={cy} r={(n/10)*maxR} fill="none" stroke="rgba(200,164,90,0.08)" strokeWidth="1"/>)}
            {/* Sector lines */}
            {WHEEL_AREAS.map((_, i) => {
              const angle = (i/8) * 2 * Math.PI - Math.PI/2;
              return <line key={i} x1={cx} y1={cy} x2={cx+maxR*Math.cos(angle)} y2={cy+maxR*Math.sin(angle)} stroke="rgba(200,164,90,0.1)" strokeWidth="1"/>;
            })}
            {/* Filled sectors */}
            {WHEEL_AREAS.map((area, i) => (
              <path key={area} d={wheelPaths[i]} fill={WHEEL_COLORS[i]} fillOpacity="0.5" stroke={WHEEL_COLORS[i]} strokeWidth="1.5"/>
            ))}
            {/* Labels */}
            {labelPositions.map((p, i) => (
              <text key={i} x={p.x} y={p.y} fontSize="20" textAnchor="middle" dominantBaseline="middle">{p.text}</text>
            ))}
            {/* Center */}
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {WHEEL_AREAS.map((area, i) => (
            <div key={area} style={{display:"flex",alignItems:"center",gap:10}}>
              <span style={{fontSize:18,width:24}}>{AREA_EMOJI[area]||"⭐"}</span>
              <span style={{flex:1,fontSize:14,color:"#C8C4BC",fontFamily:"'Crimson Pro',serif"}}>{area}</span>
              <input type="range" min="1" max="10" value={wheelScores[area]||5} onChange={e=>updateScore(area, parseInt(e.target.value))} style={{flex:2,accentColor:WHEEL_COLORS[i]}}/>
              <span style={{fontFamily:"'JetBrains Mono'",fontSize:13,color:WHEEL_COLORS[i],width:24,textAlign:"right"}}>{wheelScores[area]||5}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <AiBox kb={kb} prompt={"Составь персональный план достижения цели для "+( profile.name||"меня")+". Главная цель: "+(profile.mainGoal||"не указана")+". Приоритетные сферы: "+(goalAreas.join(",")||"—")+". Что мешает: "+(goalBlocks.join(",")||"—")+". Мотивирует: "+(profile.motivates||"—")+". Ключевая ценность: "+(profile.coreValue||"—")+". Свободное время: с "+(profile.workEnd||"18:00")+" до "+(profile.sleep||"23:00")+", "+(profile.selfTime||"30")+" мин/день на себя. Луна: "+moon.n+"("+moon.t+"). Дай ответ с заголовками ##: ## Декомпозиция цели — 3 шага на неделю; ## Преодоление блоков — по каждому блоку конкретно; ## Утренняя аффирмация; ## Что сделать сегодня за 15 минут."} label="План достижения цели" btnText="Составить мой план" placeholder="Составлю персональный план под твою цель и ритм жизни..."/>

      <AiBox kb={kb} prompt={"Проанализируй колесо жизни "+( profile.name||"меня")+". Мои оценки сфер (1-10): "+WHEEL_AREAS.map(a=>a+":"+(wheelScores[a]||5)).join(", ")+". Приоритеты: "+(goalAreas.join(",")||"—")+". Главная цель: "+(profile.mainGoal||"—")+". Истощает: "+((profile.workDrain||[]).join(",")||"—")+". Мотивирует: "+(profile.motivates||"—")+". Дай ответ с заголовками ##: ## Общая оценка баланса; ## Самые слабые сферы — что с ними делать; ## Самые сильные сферы — как использовать; ## Действие на эту неделю по каждой провисающей сфере (нумерованный список)."} label="Анализ моего колеса" btnText="Проанализировать колесо" placeholder="Проанализирую твой баланс жизненных сфер..."/>
    ...
    <div>
            ...
            <div style={{display:"flex",gap:16,alignItems:"flex-start"}}>
              ...
              <div>
                ...
                <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
                  ...
                  {profile.dob&&<span className="badge bm">{calcAge(profile.dob)} лет</span>}
      ...
      {view==="astro"&&(
        <div>
          ...
          <AiBox kb={kb} prompt={"Составь подробный астрологический и нумерологический портрет для "+(profile.fullName||profile.name||"меня")+". ФИО: "+(profile.fullName||profile.name||"—")+", дата рождения: "+(profile.dob||"—")+", возраст: "+(calcAge(profile.dob)||"—")+" лет. Знак зодиака: "+z.name+", восточный знак: "+east+", градус судьбы по ФИО: "+(deg||"—")+"°, число судьбы: "+(numerologyDestiny(profile.dob)||"—")+". ВАЖНО: используй имя \""+(profile.name||"")+"\" в обращении и опирайся на конкретные расчёты по ФИО и дате. Дай ответ ТОЛЬКО с заголовками формата ## **Заголовок** и нумерованными пунктами без markdown bold. Структура: 1) ## Характер и личность — 4 пункта, 2) ## Сильные стороны — 4 пункта, 3) ## Слабые места — 3 пункта, 4) ## Здоровье — 3 пункта (под зоны "+((profile.healthFocus||[]).join(",")||"общее")+"), 5) ## Любовь и отношения — 3 пункта, 6) ## Карьера и финансы — 3 пункта."} label="Персональный астропортрет" btnText="Составить мой портрет" placeholder="Получи свой полный астрологический портрет..."/>
          <AiBox kb={kb} prompt={"Расшифруй жизненный путь "+(profile.name||"меня")+" по периодам. ВАЖНО: используй РЕАЛЬНЫЕ годы рождения и факты. ФИО: "+(profile.fullName||profile.name||"—")+", дата рождения: "+(profile.dob||"—")+" (родилась в "+(profile.dob?new Date(profile.dob).getFullYear():"—")+" году), сейчас "+(calcAge(profile.dob)||"—")+" лет. Знак: "+z.name+", восточный: "+east+", градус: "+(deg||"—")+"°, число судьбы: "+(numerologyDestiny(profile.dob)||"—")+". Дай ответ с заголовками ## и нумерованными пунктами. Структура: ## Детство ("+(profile.dob?new Date(profile.dob).getFullYear():"")+"-"+(profile.dob?new Date(profile.dob).getFullYear()+12:"")+") — формирование личности, какой была "+(profile.name||"")+", 4 пункта; ## Юность ("+(profile.dob?new Date(profile.dob).getFullYear()+13:"")+"-"+(profile.dob?new Date(profile.dob).getFullYear()+21:"")+") — становление, 4 пункта; ## Молодость ("+(profile.dob?new Date(profile.dob).getFullYear()+22:"")+"-"+(profile.dob?new Date(profile.dob).getFullYear()+35:"")+") — главные уроки, 4 пункта; ## Зрелость ("+(profile.dob?new Date(profile.dob).getFullYear()+36:"")+"-"+(profile.dob?new Date(profile.dob).getFullYear()+50:"")+") — пик реализации, текущий период если возраст в этом диапазоне, 4 пункта; ## Мудрость ("+(profile.dob?new Date(profile.dob).getFullYear()+51:"")+"+) — что впереди, 3 пункта."} label="Жизненный путь по годам" btnText="Расшифровать" placeholder="Покажу твой жизненный путь по этапам..."/>
          <AiBox kb={kb} prompt={"Рассчитай ключевые годы жизни для "+(profile.name||"меня")+" на основе нумерологии и астрологических циклов. ФИО: "+(profile.fullName||profile.name||"—")+", дата рождения: "+(profile.dob||"—")+", родилась в "+(profile.dob?new Date(profile.dob).getFullYear():"—")+" году, сейчас "+(calcAge(profile.dob)||"—")+" лет (год: "+(new Date().getFullYear())+"). Личный год сейчас: "+(personalYear(profile.dob, new Date().getFullYear())||"—")+". Возвращение Сатурна (~30 лет) было в "+(profile.dob?new Date(profile.dob).getFullYear()+30:"—")+", второе будет в "+(profile.dob?new Date(profile.dob).getFullYear()+59:"—")+". Циклы Юпитера (12 лет): "+(profile.dob?(new Date(profile.dob).getFullYear()+12)+", "+(new Date(profile.dob).getFullYear()+24)+", "+(new Date(profile.dob).getFullYear()+36)+", "+(new Date(profile.dob).getFullYear()+48):"—")+". Дай ответ с заголовками ##: ## Прошлые ключевые годы (3-4 конкретных года из жизни с пояснением что это были за периоды); ## "+new Date().getFullYear()+" год (личный год "+(personalYear(profile.dob, new Date().getFullYear())||"—")+", тема и события); ## "+(new Date().getFullYear()+1)+" год; ## "+(new Date().getFullYear()+2)+" год; ## "+(new Date().getFullYear()+3)+" год; ## Дальняя перспектива (важные годы 5-10 лет вперёд). По каждому году конкретные месяцы и события с нумерацией пунктов."} label="Ключевые даты жизни" btnText="Показать мои даты" placeholder="Покажу важные годы и события прошлого и будущего..."/>
        </div>
      )}
      {view==="sections"&&(
        <div className="card">
