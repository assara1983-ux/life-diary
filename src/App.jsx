...
export default function LifeDiary() {
  ...
  const [weekMenu, setWeekMenu] = useStorage("ld_week_menu_v1", null);
  // weekMenu: { generatedAt: ISO, days: [{date, dayName, breakfast, lunch, dinner}], weekStart, weekEnd }
  ...
  // ── Авто-итог дня в 23:00 ──────────────────────────────────
  useEffect(()=>{
    if(!profile) return;
    const checkAutoSummary = async () => {
      const now = new Date();
      const h = now.getHours();
      const todayKey = toDay();
      const summaryKey = "ld_summary_done_" + todayKey;
      if(h >= 23 && !localStorage.getItem(summaryKey)) {
        localStorage.setItem(summaryKey, "1");
        // Собираем данные за день
        const doneTasks = tasks.filter(t => t.doneDate === todayKey);
        const journalEntry = journal[todayKey] || {};
        const shopDone = shopList.filter(x => x.done);
        const summaryText = [
          "📋 Итог дня " + new Date().toLocaleDateString("ru-RU",{weekday:"long",day:"numeric",month:"long"}),
          doneTasks.length > 0 ? "✅ Выполнено задач: " + doneTasks.length + " — " + doneTasks.slice(0,5).map(t=>t.title).join(", ") + (doneTasks.length>5?"...":"") : "📋 Задач не выполнено",
          journalEntry.mood ? "😊 Настроение: " + journalEntry.mood : "",
          journalEntry.energy ? "⚡ Энергия: " + journalEntry.energy + "/5" : "",
          journalEntry.win ? "✦ Победа дня: " + journalEntry.win : "",
          journalEntry.gratitude ? "🙏 Благодарность: " + journalEntry.gratitude : "",
          shopDone.length > 0 ? "🛒 Куплено: " + shopDone.slice(0,3).map(x=>x.name).join(", ") + (shopDone.length>3?"...":"") : "",
          "🤖 Авто-сохранено в 23:00"
        ].filter(Boolean).join("\n");
        setJournal(prev => ({
          ...prev,
          [todayKey]: {
            ...
            autoSummaryAt: now.toISOString()
          }
        }));
        notify("Итог дня сохранён в журнал ✦");
      }
    };
    checkAutoSummary();
    const interval = setInterval(checkAutoSummary, 60000);
    return () => clearInterval(interval);
  }, [profile, tasks, journal, shopList]);

  // ── Напоминание о новом меню на следующую неделю (воскр. после 19:00) ──
  useEffect(()=>{
    if(!profile || !weekMenu) return;
    const now = new Date();
    const dow = now.getDay(); // 0=вс
    const h = now.getHours();
    const reminderKey = "ld_menu_reminder_" + toDay();
    if(dow === 0 && h >= 19 && !localStorage.getItem(reminderKey)) {
      localStorage.setItem(reminderKey, "1");
      const nextMon = new Date(now);
      nextMon.setDate(now.getDate() + 1);
      const nextMonStr = nextMon.toLocaleDateString("ru-RU",{day:"numeric",month:"long"});
      setJournal(prev => {
        const todayKey = toDay();
        return {
          ...prev,
          [todayKey]: {
            ...(prev[todayKey]||{}),
            menuReminder: "🍽 Напоминание: составь меню на следующую неделю (с " + nextMonStr + "). Открой раздел Здоровье → Меню на неделю.",
            menuReminderAt: now.toISOString()
          }
        };
      });
      notify("Пора составить меню на следующую неделю 🍽");
    }
  }, [profile, weekMenu]);
      ...
      <div className="app">
        ...
        <div className="main">
          ...
          <div className="page">
            ...
            {active==="health"   && <HealthSection profile={profile} tasks={tasks} setTasks={setTasks} setShopList={setShopList} setJournal={setJournal} weekMenu={weekMenu} setWeekMenu={setWeekMenu} today={today} kb={kb} notify={notify}/>}
...
function AiBox({ kb, prompt, label="ИИ-СОВЕТНИК", btnText="Получить совет", placeholder="Нажми — получи персональный совет...", actionType=null, onShopAdd=null, onTaskAdd=null, noActions=false }) {
...
//  HEALTH
// ══════════════════════════════════════════════════════════════
// ── Хелпер: определить диапазон дней меню от текущего дня до конца недели ──
function getMenuRange() {
  const now = new Date();
  ...
  // Конец недели = ближайшее воскресенье включительно
  // Если сегодня воскресенье (0) — конец = эта суббота+7, т.е. следующая вс
  ...
  while(cur <= end) {
    days.push({ date: toDay(cur), dayName: DAY_NAMES[cur.getDay()] });
    cur.setDate(cur.getDate() + 1);
  }
  return { days, startStr: start.toLocaleDateString("ru-RU",{day:"numeric",month:"long"}), endStr: end.toLocaleDateString("ru-RU",{day:"numeric",month:"long"}) };
}

// ── Хелпер: какой день меню показывать прямо сейчас ──
function getTodayMenuDay(weekMenu) {
  if(!weekMenu || !weekMenu.days) return null;
  const now = new Date();
  const h = now.getHours();
  const todayKey = toDay();
  // После 19:00 показываем меню на завтра
  if(h >= 19) {
    const tomorrow = new Date(now);
    tomorrow.setDate(now.getDate() + 1);
    const tmKey = toDay(tomorrow);
    return weekMenu.days.find(d => d.date === tmKey) || null;
  }
  // До 18:30 — меню на сегодня
  return weekMenu.days.find(d => d.date === todayKey) || null;
}

function HealthSection({profile,tasks,setTasks,setShopList,setJournal,weekMenu,setWeekMenu,today,kb,notify}) {
  const [modal,setModal]=useState(null);
  const [menuView,setMenuView]=useState("today"); // "today"|"week"
  const [menuLoading,setMenuLoading]=useState(false);
  const [editDay,setEditDay]=useState(null); // {date, dayName, breakfast, lunch, dinner}
  const moon=getMoon();
  ...
  const generateMenu = async () => {
    setMenuLoading(true);
    const { days, startStr, endStr } = getMenuRange();
    if(days.length === 0) { notify("На этой неделе дней не осталось"); setMenuLoading(false); return; }
    const dayList = days.map(d => d.dayName).join(", ");
    const prompt = "Составь меню питания строго в JSON. Отвечай ТОЛЬКО валидным JSON без пояснений, без markdown, без текста до и после JSON.\n\nТип питания: "+(profile.nutrition||"обычное")+". Зоны здоровья: "+((profile.healthFocus||[]).join(","))+". Хронические: "+(profile.chronic||"нет")+". Цель: "+(profile.healthGoal||"—")+". Всегда дома: "+((profile.staples||[]).join(","))+". Луна: "+moonN+". Сезон: "+season+".\n\nДни для меню: "+dayList+".\n\nФормат ответа — JSON массив объектов:\n[{\"dayName\":\"Понедельник\",\"breakfast\":\"Овсянка с ягодами, зелёный чай\",\"lunch\":\"Куриный суп, хлеб\",\"dinner\":\"Запечённая рыба с овощами\",\"shopItems\":[\"[Продукты] Куриное филе 300г\",\"[Продукты] Рыба 200г\"]}]\n\nКаждый элемент массива соответствует одному дню. Поле shopItems — список покупок для этого дня с метками [Продукты] или [Аптека].";
    const raw = await askClaude(kb, prompt, 1800);
    try {
      ...
      const newDays = days.map((d,i) => ({
        ...
        lunch: arr[i]?.lunch || "",
        dinner: arr[i]?.dinner || "",
        shopItems: arr[i]?.shopItems || []
      }));
      const menuObj = { generatedAt: new Date().toISOString(), days: newDays, startStr, endStr };
      setWeekMenu(menuObj);
      // Сохранить в журнал
      const todayKey = toDay();
      const menuJournalText = "🍽 Меню на неделю ("+startStr+" — "+endStr+") составлено AI.\n" + newDays.map(d=>"• "+d.dayName+": "+d.breakfast+" / "+d.lunch+" / "+d.dinner).join("\n");
      setJournal(prev => ({
        ...prev,
        [todayKey]: { ...(prev[todayKey]||{}), weekMenuSaved: menuJournalText, weekMenuAt: new Date().toISOString() }
      }));
      notify("Меню на неделю составлено и сохранено ✦");
    } catch(e) {
      // Если AI вернул не JSON — сохраняем как текст
      notify("Меню получено, но не удалось разобрать структуру. Попробуй ещё раз.");
    }
    setMenuLoading(false);
  };

  // Добавить продукты конкретного дня в покупки
  const addDayToShopping = (day) => {
    if(!day || !day.shopItems || day.shopItems.length === 0) { notify("Нет продуктов для добавления"); return; }
    const cats = ["Продукты","Бытовая химия","Красота и уход","Для питомцев","Одежда","Аптека","Другое"];
    const newItems = day.shopItems.map(raw => {
      const m = raw.match(/\[([^\]]+)\]/);
      const cat = (m && cats.includes(m[1])) ? m[1] : "Продукты";
      const name = raw.replace(/\[[^\]]+\]/g,"").replace(/^[:\s—\-]+/,"").trim();
      return name ? { id: Date.now()+Math.random(), name, cat, done: false } : null;
    }).filter(Boolean);
    if(newItems.length === 0) { notify("Нет продуктов"); return; }
    setShopList(p => [...p, ...newItems]);
    notify("Добавлено " + newItems.length + " продуктов в покупки 🛒");
  };

  // Добавить ВСЕ продукты недели
  const addAllToShopping = () => {
    if(!weekMenu) return;
    const allItems = weekMenu.days.flatMap(d => d.shopItems || []);
    const cats = ["Продукты","Бытовая химия","Красота и уход","Для питомцев","Одежда","Аптека","Другое"];
    const seen = new Set();
    const newItems = allItems.map(raw => {
      const m = raw.match(/\[([^\]]+)\]/);
      const cat = (m && cats.includes(m[1])) ? m[1] : "Продукты";
      const name = raw.replace(/\[[^\]]+\]/g,"").replace(/^[:\s—\-]+/,"").trim();
      if(!name || seen.has(name.toLowerCase())) return null;
      seen.add(name.toLowerCase());
      return { id: Date.now()+Math.random(), name, cat, done: false };
    }).filter(Boolean);
    setShopList(p => [...p, ...newItems]);
    notify("Добавлено " + newItems.length + " продуктов в покупки 🛒");
  };

  const todayMenuDay = weekMenu ? getTodayMenuDay(weekMenu) : null;
  const now = new Date();
  const showingTomorrow = now.getHours() >= 19;
  ...
  return(
    <div>
      ...
      {/* ── МЕНЮ ── */}
      <div className="card" style={{borderColor:T.bdrH}}>
        <div className="card-hd">
          <div className="card-title">🍽 Меню питания</div>
          <div className="btn-row">
            <div className="tabs" style={{margin:0}}>
              {[["today","Сегодня"],["week","Вся неделя"]].map(([v,l])=>(
                <div key={v} className={`tab${menuView===v?" on":""}`} style={{fontSize:12,padding:"5px 12px"}} onClick={()=>setMenuView(v)}>{l}</div>
              ))}
            </div>
          </div>
        </div>
        ...
        {!weekMenu && (
          <div>
            <div className="empty" style={{marginBottom:12}}>
              ...
              <p>Меню на неделю ещё не составлено</p>
              <p style={{fontSize:12,color:T.text3,marginTop:4}}>AI составит меню под твой профиль и цели здоровья</p>
            </div>
            <button className="btn btn-primary" style={{width:"100%"}} onClick={generateMenu} disabled={menuLoading}>
              {menuLoading ? "Составляю меню..." : "✦ Составить меню на неделю"}
            </button>
          </div>
        )}

        {/* Есть меню — вид "сегодня" */}
        {weekMenu && menuView === "today" && (
          <div>
            {todayMenuDay ? (
              <div>
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
                  <span style={{fontFamily:"'Cormorant Infant',serif",fontSize:20,color:T.gold}}>{showingTomorrow ? "Завтра — " : ""}{todayMenuDay.dayName}</span>
                  {showingTomorrow && <span style={{fontSize:11,color:T.teal,fontFamily:"'JetBrains Mono'",letterSpacing:.5}}>меню на завтра (после 19:00)</span>}
                </div>
                {[["🌅 Завтрак","breakfast"],["☀️ Обед","lunch"],["🌙 Ужин","dinner"]].map(([lbl,key])=>(
                  <div key={key} style={{marginBottom:10,padding:"10px 14px",background:"rgba(255,255,255,0.03)",borderRadius:10,border:`1px solid ${T.bdrS}`}}>
                    <div style={{fontSize:11,color:T.text3,fontFamily:"'JetBrains Mono'",letterSpacing:1,marginBottom:4}}>{lbl}</div>
                    {editDay && editDay.date===todayMenuDay.date ? (
                      <input style={{width:"100%",background:"transparent",border:"none",borderBottom:`1px solid ${T.bdr}`,color:T.text0,fontFamily:"'Crimson Pro',serif",fontSize:15,outline:"none",padding:"4px 0"}}
                        value={editDay[key]||""} onChange={e=>setEditDay(p=>({...p,[key]:e.target.value}))}/>
                    ) : (
                      <div style={{fontSize:15,color:T.text0,lineHeight:1.5}}>{todayMenuDay[key]||"—"}</div>
                    )}
                  </div>
                ))}
                <div style={{display:"flex",gap:8,marginTop:10,flexWrap:"wrap"}}>
                  {!editDay && <button className="btn btn-ghost btn-sm" onClick={()=>setEditDay({...todayMenuDay})}>✏️ Редактировать</button>}
                  {editDay && editDay.date===todayMenuDay.date && <>
                    <button className="btn btn-primary btn-sm" onClick={()=>{
                      setWeekMenu(p=>({...p,days:p.days.map(d=>d.date===editDay.date?editDay:d)}));
                      setEditDay(null); notify("Меню обновлено");
                    }}>Сохранить</button>
                    <button className="btn btn-ghost btn-sm" onClick={()=>setEditDay(null)}>Отмена</button>
                  </>}
                  <button className="btn btn-ghost btn-sm" onClick={()=>addDayToShopping(todayMenuDay)}>🛒 В покупки</button>
                </div>
              </div>
            ) : (
              <div>
                <div className="empty"><span className="empty-ico">📅</span><p>Этот день не входит в текущее меню</p></div>
                <button className="btn btn-ghost btn-sm" style={{marginTop:8}} onClick={()=>setMenuView("week")}>Посмотреть всё меню</button>
              </div>
            )}
            <div style={{marginTop:14,padding:"8px 12px",background:"rgba(78,201,190,0.05)",borderRadius:9,fontSize:12,color:T.text3,display:"flex",alignItems:"center",gap:8}}>
              <span>📅</span>
              <span>Меню действует: {weekMenu.startStr} — {weekMenu.endStr} · После 19:00 показывается меню на завтра</span>
            </div>
          </div>
        )}

        {/* Вид "вся неделя" */}
        {weekMenu && menuView === "week" && (
          <div>
            <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap"}}>
              <button className="btn btn-ghost btn-sm" onClick={addAllToShopping}>🛒 Все продукты в покупки</button>
              <button className="btn btn-ghost btn-sm" onClick={()=>{if(window.confirm("Пересоставить меню?"))generateMenu();}} disabled={menuLoading}>
                {menuLoading?"Составляю...":"🔄 Пересоставить"}
              </button>
            </div>
            {weekMenu.days.map(day=>(
              <div key={day.date} style={{marginBottom:12,padding:"12px 16px",background:"rgba(255,255,255,0.02)",border:`1px solid ${day.date===today?T.gold+"44":T.bdrS}`,borderRadius:12}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                  <span style={{fontFamily:"'Cormorant Infant',serif",fontSize:17,color:day.date===today?T.gold:T.text0}}>
                    {day.date===today?"★ ":""}{day.dayName}
                    <span style={{fontSize:11,color:T.text3,marginLeft:8,fontFamily:"'JetBrains Mono'"}}>{new Date(day.date+"T12:00:00").toLocaleDateString("ru-RU",{day:"numeric",month:"short"})}</span>
                  </span>
                  <div style={{display:"flex",gap:6}}>
                    <button className="btn-mini" onClick={()=>setEditDay(editDay?.date===day.date?null:{...day})}>✏️</button>
                    <button className="btn-mini" onClick={()=>addDayToShopping(day)}>🛒</button>
                  </div>
                </div>
                {editDay && editDay.date===day.date ? (
                  <div>
                    {[["Завтрак","breakfast"],["Обед","lunch"],["Ужин","dinner"]].map(([lbl,key])=>(
                      <div key={key} className="fld" style={{marginBottom:6}}>
                        <label style={{fontSize:11}}>{lbl}</label>
                        <input value={editDay[key]||""} onChange={e=>setEditDay(p=>({...p,[key]:e.target.value}))} style={{fontSize:14}}/>
                      </div>
                    ))}
                    <div style={{display:"flex",gap:8}}>
                      <button className="btn btn-primary btn-sm" onClick={()=>{
                        setWeekMenu(p=>({...p,days:p.days.map(d=>d.date===editDay.date?editDay:d)}));
                        setEditDay(null); notify("Сохранено");
                      }}>Сохранить</button>
                      <button className="btn btn-ghost btn-sm" onClick={()=>setEditDay(null)}>Отмена</button>
                    </div>
                  </div>
                  ...
                  <div style={{display:"grid",gap:4}}>
                    {[["🌅","breakfast"],["☀️","lunch"],["🌙","dinner"]].map(([ico,key])=>(
                      <div key={key} style={{fontSize:13,color:T.text1,display:"flex",gap:7}}>
                        <span>{ico}</span><span>{day[key]||"—"}</span>
                      </div>
                    ))}
                  </div>
                )}
            ...
            <div style={{marginTop:8,padding:"8px 12px",background:"rgba(200,164,90,0.05)",borderRadius:9,fontSize:12,color:T.text3}}>
              💾 Меню сохранено в журнал {new Date(weekMenu.generatedAt).toLocaleDateString("ru-RU",{day:"numeric",month:"long",hour:"2-digit",minute:"2-digit"})}
            </div>
          </div>
        )}
      </div>
...
function GoalsSection({profile,kb,notify}) {
  ...
  const WHEEL_AREAS = [
    ...
    {name:"Отношения", emoji:"❤️", color:"#E87878", desc:"Партнёр, друзья, близкое окружение"},
    {name:"Семья", emoji:"👨‍👩‍👧", color:"#B882E8", desc:"Родители, дети, домашняя атмосфера"},
    {name:"Саморазвитие", emoji:"📚", color:"#7EDDD5", desc:"Обучение, рост, новые навыки"},
    {name:"Творчество", emoji:"🎨", color:"#E8A85A", desc:"Хобби, самовыражение, увлечения"},
    {name:"Духовность", emoji:"🌟", color:"#C8C4BC", desc:"Ценности, смысл жизни, внутренний мир"}
  ];
      ...
      <div className="card" style={{marginBottom:12}}>
        ...
        <div style={{display:"flex",justifyContent:"center",marginBottom:14}}>
          <svg width="360" height="360" viewBox="-30 -30 380 380" style={{maxWidth:"100%"}}>
            ...
            {[2,4,6,8,10].map(n=><circle key={n} cx={cx} cy={cy} r={(n/10)*maxR} fill="none" stroke="rgba(200,164,90,0.1)" strokeWidth="1"/>)}
        ...
        {activeArea !== null && <div style={{padding:"14px 16px",background:"linear-gradient(135deg,"+WHEEL_AREAS[activeArea].color+"22,"+WHEEL_AREAS[activeArea].color+"08)",border:"1px solid "+WHEEL_AREAS[activeArea].color+"55",borderRadius:12,marginBottom:14}}>
          ...
          <div style={{fontSize:14,color:"#C8C4BC",lineHeight:1.6,marginBottom:12,fontFamily:"'Crimson Pro',serif",fontStyle:"italic"}}>{WHEEL_AREAS[activeArea].desc}</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:6,marginTop:8}}>
          {WHEEL_AREAS.map((area,i)=>(
            <div key={area.name} onClick={()=>setActiveArea(i)} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 10px",borderRadius:8,background:activeArea===i?area.color+"22":"transparent",border:"1px solid "+(activeArea===i?area.color:"transparent"),cursor:"pointer",transition:"all .15s"}}>
              ...
              <span style={{flex:1,fontSize:12,color:"#C8C4BC"}}>{area.name}</span>
    ...
    <div>
      ...
      {view==="today"&&(
        <div>
          {/* Авто-итог дня */}
          {e.autoSummary&&(
            <div className="card" style={{marginBottom:8,border:`1px solid ${T.gold}33`,background:"rgba(200,164,90,0.04)"}}>
              <div style={{fontFamily:"'JetBrains Mono'",fontSize:9,color:T.goldD,letterSpacing:2,marginBottom:8,textTransform:"uppercase"}}>Авто-итог дня · {new Date(e.autoSummaryAt||today).toLocaleTimeString("ru-RU",{hour:"2-digit",minute:"2-digit"})}</div>
              <div style={{fontSize:13,color:T.text1,lineHeight:1.7,whiteSpace:"pre-line"}}>{e.autoSummary}</div>
            </div>
          )}
          {/* Напоминание о меню */}
          {e.menuReminder&&(
            <div className="card" style={{marginBottom:8,border:`1px solid ${T.teal}33`,background:"rgba(78,201,190,0.04)"}}>
              <div style={{fontSize:13,color:T.teal,lineHeight:1.6}}>{e.menuReminder}</div>
            </div>
          )}
          {/* Меню недели сохранено */}
          {e.weekMenuSaved&&(
            <div className="card" style={{marginBottom:8,border:`1px solid rgba(91,173,122,0.3)`,background:"rgba(91,173,122,0.04)"}}>
              <div style={{fontFamily:"'JetBrains Mono'",fontSize:9,color:T.success,letterSpacing:2,marginBottom:8,textTransform:"uppercase"}}>Меню сохранено в журнал</div>
              <div style={{fontSize:12,color:T.text2,lineHeight:1.6,whiteSpace:"pre-line"}}>{e.weekMenuSaved}</div>
            </div>
          )}
          <div className="card">
