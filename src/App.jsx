...
@media(max-width:700px) {
  ...
  /* Main content fills full width, padded above bottom bar */
      ...
      {detailItem && <div className="overlay" onClick={()=>setDetailItem(null)}>
        <div className="modal" onClick={e=>e.stopPropagation()} style={{maxWidth:600,maxHeight:"92vh",overflowY:"auto"}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:18}}>
            <button className="btn btn-ghost btn-sm" style={{flexShrink:0,padding:"6px 12px"}} onClick={()=>setDetailItem(null)}>← Назад</button>
            <div className="modal-title" style={{margin:0,flex:1}}>Подробная инструкция</div>
            <span className="modal-x" style={{position:"static",flexShrink:0}} onClick={()=>setDetailItem(null)}>✕</span>
          </div>
          ...
          {!detailLoading && <div style={{marginTop:20,paddingTop:16,borderTop:`1px solid ${T.bdr}`}}>
            <button className="btn btn-ghost" style={{width:"100%"}} onClick={()=>setDetailItem(null)}>← Закрыть</button>
          </div>}
        </div>
      </div>}
