...
// ✅ ОБЪЕДИНЁННЫЙ РАЗДЕЛ: заменяет health и mental
{ id: "health_mental", emoji: "🫁", name: "Здоровье & Ментальное", vis: true },
...
const [healthAdvice, setHealthAdvice] = useStorageState('ld_health_advice', true);const [healthHabits, setHealthHabits] = useStorageState('ld_health_habits', true);
// ✅ NOTIFY — глобальные уведомления (ИСПРАВЛЕНО: было undefined во всех секциях)
const [toastMsg, setToastMsg] = useState('');
const notify = useCallback((msg) => {
setToastMsg(msg);
setTimeout(() => setToastMsg(''), 2500);
}, []);
// --- УМНАЯ СИНХРОНИЗАЦИЯ ЗАДАЧ (ИСПРАВЛЕНО: убрана зависимость от tasks для предотвращения цикла) ---
...
// ✅ ИСПРАВЛЕНО: tasks убран из зависимостей — функциональный сеттер решает проблему цикла
}, [selectedReports, customReportGroups, allCatalogReports, setTasks]);
// --- Экспорт данных ---
const value = {profile, setProfile, sections, setSections, tasks, setTasks,
...
// ✅ ИСПРАВЛЕНО: notify теперь экспортируется
notify,
// ✅ Toast сообщение для отображения в App.jsx
toastMsg,
};
