...
export function calculateNextDeadline(frequency, lastDeadline) {
  if (!lastDeadline) lastDeadline = new Date().toISOString().split('T')[0];
...
export function AppProvider({ children }) {
  ...
  const [sections, setSections] = useStorageState('ld_sec_v3', [
    {id:"today",emoji:"☀️",name:"Сегодня",vis:true},{id:"schedule",emoji:"🗓️",name:"Расписание",vis:true},
    {id:"work",emoji:"💼",name:"Работа",vis:true},{id:"home",emoji:"🏡",name:"Дом",vis:true},
    {id:"shopping",emoji:"🛒",name:"Покупки",vis:true},{id:"pets",emoji:"🐾",name:"Питомцы",vis:true},
    ...
    {id:"goals",emoji:"🎯",name:"Мои цели",vis:true},{id:"mental",emoji:"🧘",name:"Ментальное",vis:true},    {id:"travel",emoji:"✈️",name:"Поездки",vis:true},{id:"journal",emoji:"📖",name:"Журнал",vis:true},
  ...
  const [goalsTools, setGoalsTools] = useStorageState('ld_goals_tools', {weightLog:[],habits:[],calories:{goal:0,log:[]},workout:null,workoutSetup:null});
  ...
  const [beautyTodayOpen, setBeautyTodayOpen] = useStorageState('ld_beauty_today_open', true);  const [beautyChooseOpen, setBeautyChooseOpen] = useStorageState('ld_beauty_choose_open', true);
  ...
  const syncReportsToTasks = useCallback(() => {
    ...
    accountingReports.forEach((report) => {
      ...
      if (daysLeft <= 5 && daysLeft >= 0) {
        ...
        if (!taskExists) {
          newTasks.push({
            id: `report-${report.id}-${report.nextDeadline}`, type: 'report', reportId: report.id,
            title: `📋 ${report.name}`, section: 'work', deadline: report.nextDeadline,
            priority: daysLeft <= 1 ? 'h' : daysLeft <= 3 ? 'm' : 'l',
            notes: `Срок сдачи: ${report.deadline}`, freq: 'once', lastDone: '', doneDate: null,
      ...
      if (report.status === 'done' && report.nextDeadline && daysUntilDeadline(report.nextDeadline) <= 0) {
        ...
        setAccountingReports((prev) => prev.map((r) => r.id === report.id ? { ...r, status: 'pending', nextDeadline: newNextDeadline, lastCompleted: report.nextDeadline } : r));
      }
    });
  ...
  useEffect(() => { if (accountingReports.length > 0) syncReportsToTasks(); }, [accountingReports, syncReportsToTasks]);

  const value = {
    profile, setProfile, sections, setSections, tasks, setTasks, journal, setJournal,
    shopList, setShopList, petLog, setPetLog, trips, setTrips, hobbies, setHobbies,
    reportGroups, setReportGroups, reports, setReports, workTools, setWorkTools,
    checkResults, setCheckResults, accountingReports, setAccountingReports,
    goalsTools, setGoalsTools, wheelScores, setWheelScores,
    weekMenu, setWeekMenu, beautyProcs, setBeautyProcs, beautyTopics, setBeautyTopics,
    feedTimes, setFeedTimes, commuteSettings, setCommuteSettings,
    ...
    workOpenGroups, setWorkOpenGroups, workOpenTasks, setWorkOpenTasks,    workOpenAdvice, setWorkOpenAdvice, shopAdvice, setShopAdvice,
    shopListOpen, setShopListOpen, petsAdvice, setPetsAdvice,
    petsFeed, setPetsFeed, petsCare, setPetsCare,
    ...
    beautyChooseOpen, setBeautyChooseOpen, healthAdvice, setHealthAdvice,
